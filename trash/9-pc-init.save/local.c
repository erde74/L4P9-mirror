...
int connect_ramfs(void)
{
    return  1;
}

#if 0 //%---------------------------------------
int connectlocalkfs(void)
{
	int i, pid, fd, p[2];
	char partition[64];
	char *dev;
...
static int print1(int fd, char *s)
{
...
#if 1 //%-------------------------------
	if((fd = connect_dosfs()) < 0)
	if((fd = connect_ramfs()) < 0)
#else //%-----------------------------
