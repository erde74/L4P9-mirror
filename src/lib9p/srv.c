...
#if 1 //%--------------------
void print9pmsg(Fcall *fp);  //% Added in LP49
#endif //---------------------
...
filewalk(Req *r)
{
	...
	for(i=0; i<r->ifcall.nwname; i++)
	        if((f = walkfile(f, r->ifcall.wname[i])))
		        r->ofcall.wqid[i] = f->_dir.qid;   //% _dir
		else
			break;
	...
	if(f){
		r->newfid->file = f;
		r->newfid->qid = r->newfid->file->_dir.qid;  //% _dir
	}
	respond(r, nil);
}
...
sattach(Srv *srv, Req *r)
{
	...
	if(srv->tree){
		...
		incref(&r->fid->file->_ref);   //%  r->fid->file
		r->ofcall.qid = r->fid->file->_dir.qid;   //% _dir
...
ropen(Req *r, char *error)
{
	...
	if(chatty9p){
		...
		l4printf_b("%s\n", errbuf); //% write(2, errbuf, strlen(errbuf));
	}
...
srv(Srv *srv)
{
	...
	fprint(2, "starting srv{infd:%d, outfd=%d, nopipe=%d, srvfd=%d}\n",
	       srv->infd, srv->outfd, srv->nopipe, srv->srvfd); //%
