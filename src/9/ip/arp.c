...
rxmitsols(Arp *arp)
{
	...
	for(; a; a = a->nextrxt){
		ifc = a->ifc;
		assert(ifc != nil);
		if((a->rxtsrem <= 0) || !(canrlock(&ifc->_rwlock)) || (a->ifcid != ifc->ifcid)){   //% ifc
			xp = a->hold;
			a->hold = nil;
	...
	qunlock(&arp->_qlock);  //% arp	/* for icmpns */
