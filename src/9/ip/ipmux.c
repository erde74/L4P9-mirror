...
ipmuxconnect(Conv *c, char **argv, int argc)
{
	...
	wlock(&f->_rwlock);				//% _rwlock
	f->ipmux->priv = ipmuxmerge(f->ipmux->priv, mux);
	wunlock(&f->_rwlock);				//% _rwlock
...
static char*
ipmuxannounce(Conv* _x, char** _y, int _z)
{
...
ipmuxclose(Conv *c)
{
	...
	wlock(&f->_rwlock);				//% _rwlock
	ipmuxremove((Ipmux**)&(c->p->priv), r->chain);	//% (Ipmux**) measure for warning
	wunlock(&f->_rwlock);				//% _rwlock
