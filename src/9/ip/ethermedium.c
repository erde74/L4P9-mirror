...
DPG("> etherbind(%x %s)\n", ifc, _dbg_names(argv, argc));
//for(n = 0; n < argc; n++){  l4printf_b("%s :", argv[n]);} l4printf_b("}\n", argc);
	...
	DBGPRN(" %s()-60 mchan6=%s\n", __FUNCTION__, mchan6->path->s);
...
#if 0 //%----------------------------------
	{
	    static unsigned stk_v4[512], stk_arp[512], stk_v6[512];
	    unsigned  tid;
	    tid = kproc_l4("etherread4", etherread4, ifc, &stk_v4[500]);
	    DBGPRN("  etherread4(%x)\n", ifc);
	    tid = kproc_l4("recvarpproc", recvarpproc, ifc, &stk_arp[500]);
	    DBGPRN("  recvarpproc(%x)\n", ifc);
	    kproc_l4("etherread6", etherread6, ifc, &stk_v6[500]);
	    DBGPRN("  etherread6(%x)\n", ifc);
	}
...
etherbwrite(Ipifc *ifc, Block *bp, int version, uchar *ip)
{
	...
	DBGPRN("> %s(%s) version=%d, ip=%s\n", __FUNCTION__, __FILE__, version, ip);
...
static void
etheraddmulti(Ipifc *ifc, uchar *a, uchar *_x)
{
	uchar mac[6];
	char buf[64];
	Etherrock *er = ifc->arg;
...
static void
etherremmulti(Ipifc *ifc, uchar *a, uchar *_x)
{
	uchar mac[6];
	char buf[64];
	Etherrock *er = ifc->arg;
...
#if 0		  //% LP49 has no timer function yet.  2007.3.6 sato
	if(NOW - a->ctime < 1000){
		arprelease(er->f->arp, a);
		DBGPRN("< %s(%s) (1) NOW=%d, a->ctime=%d\n", __FUNCTION__, __FILE__, NOW, a->ctime);
		return;
	}
#endif
