 ...
 * (C) Bell Lab. 
 * (C2) YS	NII
 * Plan9-C -> GCC:  '-routetree." are inserted in more than 100 places.
 */
#include	"u.h"
#include	"../port/lib.h"
#include	"../pc/mem.h"		//% ../pc/
...
freeroute(Route *r)
{
	...
	r->_routetree.left = nil;       //%
	r->_routetree.right = nil;       //%
...
allocroute(int type)
{
	...
	memset(r, 0, n);
	r->_routetree.type = type;     //%
	r->_routetree.ifc = nil;       //%
	r->_routetree.ref = 1;         //%
...
addqueue(Route **q, Route *r)
{
	...
	l = allocroute(r->_routetree.type);    //%
	l->_routetree.mid = *q;                //%
	*q = l;
	l->_routetree.left = r;                //%
}
...
walkadd(Fs *f, Route **root, Route *p)
{
	...
	l = p->_routetree.left;    //%
	r = p->_routetree.right;   //%
	p->_routetree.left = 0;    //%
	p->_routetree.right = 0;   //%
	addnode(f, root, p);
	if(l)
		walkadd(f, root, l);
	if(r)
...
calcd(Route *p)
{
	...
	if(p) {
		...
		q = p->_routetree.left;     //%
		if(q)
			d = q->_routetree.depth;   //%
		q = p->_routetree.right;           //%
		if(q && q->_routetree.depth > d)   //%
			d = q->_routetree.depth;   //%
		q = p->_routetree.mid;             //%
		if(q && q->_routetree.depth > d)   //%
			d = q->_routetree.depth;   //%
		p->_routetree.depth = d+1;         //%
	}
}
...
balancetree(Route **cur)
{
	 ...
	 */
	p = *cur;
	dl = 0; if((l = p->_routetree.left)) dl = l->_routetree.depth;   //%
	dr = 0; if((r = p->_routetree.right)) dr = r->_routetree.depth;  //%

	if(dl > dr+1) {
		p->_routetree.left = l->_routetree.right;               //%
		l->_routetree.right = p;                      //%
		*cur = l;
		calcd(p);
		calcd(l);
	} else
	if(dr > dl+1) {
		p->_routetree.right = r->_routetree.left;             //%
		r->_routetree.left = p;                    //%
		*cur = r;
		calcd(p);
		calcd(r);
...
addnode(Fs *f, Route **cur, Route *new)
{
	...
	if(p == 0) {
		*cur = new;
		new->_routetree.depth = 1;            //%
		return;
	}

	switch(rangecompare(new, p)){
	case Rpreceeds:
		addnode(f, &p->_routetree.left, new);   //%
		break;
	case Rfollows:
		addnode(f, &p->_routetree.right, new);  //%
		break;
	case Rcontains:
		/*
	...
	case Requals:
		 ...
		 *  a local interface.
		 */
		if((p->_routetree.type & Rifc) == 0){    //%
			p->_routetree.type = new->_routetree.type;  //%
			p->_routetree.ifcid = -1;         //%
			copygate(p, new);
		} else if(new->_routetree.type & Rifc)   //%
			p->_routetree.ref++;             //%
		freeroute(new);
		break;
	case Rcontained:
		addnode(f, &p->_routetree.mid, new);     //%
		break;
	}
...
v4addroute(Fs *f, char *tag, uchar *a, uchar *mask, uchar *gate, int type)
{
	...
	for(h=V4H(sa); h<=eh; h++) {
		...
		memmove(p->_routetree.tag, tag, sizeof(p->_routetree.tag));     //%

		wlock(&routelock);
		addnode(f, &f->v4root[h], p);
		while((p = f->queue)) {           
		        f->queue = p->_routetree.mid;                //%
			walkadd(f, &f->v4root[h], p->_routetree.left);  //%
			freeroute(p);
		}
		wunlock(&routelock);
	}
...
v6addroute(Fs *f, char *tag, uchar *a, uchar *mask, uchar *gate, int type)
{
	...
	*/
	...
	for(h = V6H(sa); h <= eh; h++) {
		...
		while((p = f->queue)) {               //%
			f->queue = p->_routetree.mid;          //%
			walkadd(f, &f->v6root[h], p->_routetree.left);  //%
			freeroute(p);
		}
		wunlock(&routelock);
	}
...
looknode(Route **cur, Route *r)
{
	...
	for(;;){
		...
		case Rcontains:
			return 0;
		case Rpreceeds:
			cur = &p->_routetree.left;     //%
			break;
		case Rfollows:
			cur = &p->_routetree.right;   //%
			break;
		case Rcontained:
			cur = &p->_routetree.mid;    //%
			break;
		case Requals:
			return cur;
		}
...
v4delroute(Fs *f, uchar *a, uchar *mask, int dolock)
{
	...
	for(h=V4H(rt.v4.address); h<=eh; h++) {
		...
		if(r) {
			p = *r;
			if(--(p->_routetree.ref) == 0){    //%
				*r = 0;
				addqueue(&f->queue, p->_routetree.left);  //%
				addqueue(&f->queue, p->_routetree.mid);   //%
				addqueue(&f->queue, p->_routetree.right); //%
				freeroute(p);
				while((p = f->queue)) {
					f->queue = p->_routetree.mid;     //%
					walkadd(f, &f->v4root[h], p->_routetree.left);  //%
...
v4lookup(Fs *f, uchar *a, Conv *c)
{
	...
	if(c != nil && c->r != nil && c->r->_routetree.ifc != nil && c->rgen == v4routegeneration)  //%
		return c->r;
	...
	for(p=f->v4root[V4H(la)]; p;)
		if(la >= p->v4.address) {
			if(la <= p->v4.endaddress) {
				...
				p = p->_routetree.right;  //%
		} else
			p = p->_routetree.left;  //%

	if(q && (q->_routetree.ifc == nil || q->_routetree.ifcid != q->_routetree.ifc->ifcid)){  //%
		if(q->_routetree.type & Rifc) {   //%
			hnputl(gate+IPv4off, q->v4.address);
			memmove(gate, v4prefix, IPv4off);
		} else
			v4tov6(gate, q->v4.gate);
		ifc = findipifc(f, gate, q->_routetree.type);  //%
		if(ifc == nil)
			return nil;
		q->_routetree.ifc = ifc;     //%
		q->_routetree.ifcid = ifc->ifcid;  //%
	}
...
v6lookup(Fs *f, uchar *a, Conv *c)
{
	...
	if(c != nil && c->r != nil && c->r->_routetree.ifc != nil && c->rgen == v6routegeneration)   //%
		return c->r;
...
next:		;
	...
	if(q && (q->_routetree.ifc == nil || q->_routetree.ifcid != q->_routetree.ifc->ifcid)){  //%
		if(q->_routetree.type & Rifc) {                              //%
			...
			ifc = findipifc(f, q->v6.gate, q->_routetree.type);  //%
		if(ifc == nil)
			return nil;
		q->_routetree.ifc = ifc;            //%
		q->_routetree.ifcid = ifc->ifcid;   //%
...
convroute(Route *r, uchar *addr, uchar *mask, uchar *gate, char *t, int *nifc)
{
	...
	routetype(r->_routetree.type, t);  //%

	if(r->_routetree.ifc)    //%
		*nifc = r->_routetree.ifc->conv->x;  //%
	else
		*nifc = -1;
}
...
sprintroute(Route *r, Routewalk *rw)
{
	...
	p = seprint(rw->p, rw->e, rformat, addr, mask, gate, t, r->_routetree.tag, iname);  //%
	if(rw->o < 0){
		n = p - rw->p;
		if(n > -rw->o){
...
rr(Route *r, Routewalk *rw)
{
	...
	if(rr(r->_routetree.mid, rw) == 0)  //%
		return 0;

	return rr(r->_routetree.right, rw);  //%
}
...
routeflush(Fs *f, Route *r, char *tag)
{
	if(r == nil)
		return 0;
	if(routeflush(f, r->_routetree.mid, tag))  //%
		return 1;
	if(routeflush(f, r->_routetree.left, tag))  //%
		return 1;
	if(routeflush(f, r->_routetree.right, tag))  //%
		return 1;
	if((r->_routetree.type & Rifc) == 0){  //%
		if(tag == nil || strncmp(tag, r->_routetree.tag, sizeof(r->_routetree.tag)) == 0){  //%
			delroute(f, r, 0);
			return 1;
		}
	}
