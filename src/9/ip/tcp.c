...
int	tcpiput(Proto*, Ipifc*, Block*);   //% <- void
void  	tcpoutput(Conv*);
int	tcptrim(Tcpctl*, Tcp*, Block**, ushort*);
void	tcpstart(Conv*, int);
int	tcptimeout(void*);              //% <- void
void	tcpsndsyn(Conv*, Tcpctl*);
void	tcprcvwin(Conv*);
int	tcpacktimer(void*);             //% void
int	tcpkeepalive(void*);            //% void
...
void tcpsetstate(Conv *s, uchar newstate)
{
	Tcpctl *tcb;
	uchar oldstate;
	Tcppriv *tpriv;
...
static char* tcpconnect(Conv *c, char **argv, int argc)
{
	char *e;
...
static int tcpstate(Conv *c, char *state, int n)
{
	Tcpctl *s;
...
static char* tcpannounce(Conv *c, char **argv, int argc)
{
	char *e;
...
int  tcpkick(void *x)  //% ONEDD <- void
{
	...
	if(WASERROR()){   //%
	_ERR_1:
		qunlock(&s->_qlock);		//% _qlock
		NEXTERROR_RETURN(ONERR);  //%
	}
	qlock(&s->_qlock);			//% _qlock
...
void tcprcvwin(Conv *s)				/* Call with tcb locked */
{
	int w;
	Tcpctl *tcb;
...
int tcpacktimer(void *v)  //% ONERR <- void
{
	Tcpctl *tcb;
	Conv *s;
...
static void timerstate(Tcppriv *priv, Tcptimer *t, int newstate)
{
...
#if 1 //%-------------------------------------------------
			if(t->state == TcptimerDONE && t->func != nil ){
			        int  rc; 
				rc = (*t->func)(t->arg);

				if (rc == ONERR) print("tcpackproc:func() ONERR\n");
				//% How to recover/report error ?
			}
...
void tcpgo(Tcppriv *priv, Tcptimer *t)
{
	if(t == nil || t->start == 0)
		return;
...
void tcphalt(Tcppriv *priv, Tcptimer *t)
{
	if(t == nil)
		return;
...
void localclose(Conv *s, char *reason)	 /*  called with tcb locked */
{
	Tcpctl *tcb;
	Reseq *rp,*rp1;
	Tcppriv *tpriv;
...
int tcpmtu(Proto *tcp, uchar *addr, int version, int *scale)
{
	Ipifc *ifc;
	int mtu;
...
void inittcpctl(Conv *s, int mode)
{
	Tcpctl *tcb;
	Tcp4hdr* h4;
	Tcp6hdr* h6;
...
static char* tcpflag(ushort flag)
{
	static char buf[128];
...
Block * htontcp6(Tcp *tcph, Block *data, Tcp6hdr *ph, Tcpctl *tcb)
{
	int dlen;
	Tcp6hdr *h;
	ushort csum;
...
Block * htontcp4(Tcp *tcph, Block *data, Tcp4hdr *ph, Tcpctl *tcb)
{
	int dlen;
	Tcp4hdr *h;
	ushort csum;
...
int ntohtcp6(Tcp *tcph, Block **bpp)
{
	Tcp6hdr *h;
	uchar *optr;
	ushort hdrlen;
...
int ntohtcp4(Tcp *tcph, Block **bpp)
{
	Tcp4hdr *h;
	uchar *optr;
	ushort hdrlen;
...
void tcpsndsyn(Conv *s, Tcpctl *tcb)
{
...
void sndrst(Proto *tcp, uchar *source, uchar *dest, ushort length, Tcp *seg, uchar version, char *reason)
{
	Block *hbp;
	uchar rflags;
	Tcppriv *tpriv;
...
int sndsynack(Proto *tcp, Limbo *lp)
{
	Block *hbp;
	Tcp4hdr ph4;
	Tcp6hdr ph6;
...
static void limbo(Conv *s, uchar *source, uchar *dest, Tcp *seg, int version)
{
	Limbo *lp, **l;
	Tcppriv *tpriv;
	int h;
...
static void limborexmit(Proto *tcp)
{
	Tcppriv *tpriv;
	Limbo **l, *lp;
	int h;
...
static void limborst(Conv *s, Tcp *segp, uchar *src, uchar *dst, uchar version)
{
	Limbo *lp, **l;
	int h;
	Tcppriv *tpriv;
...
static Conv* tcpincoming(Conv *s, Tcp *segp, uchar *src, uchar *dst, uchar version)
{
	Conv *new;
	Tcpctl *tcb;
	Tcppriv *tpriv;
...
int seq_within(ulong x, ulong low, ulong high)
{
	if(low <= high){
		if(low <= x && x <= high)
			return 1;
...
int seq_lt(ulong x, ulong y)
{
	return (int)(x-y) < 0;
}


int seq_le(ulong x, ulong y)
{
	return (int)(x-y) <= 0;
}


int seq_gt(ulong x, ulong y)
{
	return (int)(x-y) > 0;
}


int seq_ge(ulong x, ulong y)
{
	return (int)(x-y) >= 0;
}
...
int tcpiput(Proto *tcp, Ipifc* _x, Block *bp)  //% ONERR <- void
{
	Tcp seg;
	Tcp4hdr *h4;
	Tcp6hdr *h6;
...
int tcpkeepalive(void *v)  //% ONERR <- void
{
	Tcpctl *tcb;
	Conv *s;
...
char* tcpstartka(Conv *s, char **f, int n)
{
	Tcpctl *tcb;
	int x;
...
char* tcpsetchecksum(Conv *s, char **f, int _x)
{
	Tcpctl *tcb;
...
int tcptimeout(void *arg)   //% ONERR <- void
{
	Conv *s;
	Tcpctl *tcb;
	int maxback;
...
int addreseq(Tcpctl *tcb, Tcppriv *tpriv, Tcp *seg, Block *bp, ushort length)
{
	Reseq *rp, *rp1;
	int i;
	static int once;
...
void getreseq(Tcpctl *tcb, Tcp *seg, Block **bp, ushort *length)
{
	Reseq *rp;
...
int tcptrim(Tcpctl *tcb, Tcp *seg, Block **bp, ushort *length)
{
	ushort len;
	uchar accept;
	int dupcnt, excess;
...
void tcpadvise(Proto *tcp, Block *bp, char *msg)
{
	Tcp4hdr *h4;
	Tcp6hdr *h6;
	Tcpctl *tcb;
...
char* tcpctl(Conv* c, char** f, int n)
{
...
int tcpstats(Proto *tcp, char *buf, int len)
{
	Tcppriv *priv;
	char *p, *e;
	int i;
...
void tcpsetscale(Conv *s, Tcpctl *tcb, ushort rcvscale, ushort sndscale)
{
	if(rcvscale){
