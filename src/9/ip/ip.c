...
//%  typedef struct IP		IP;   //% double definition
...
int ipoput4(Fs *f, Block *bp, int gating, int ttl, int tos, Conv *c)  //% ONERR
{
	Ipifc *ifc;
	uchar *gate;
	ulong fragoff;
	...
	if(r->_routetree.type & (Rifc|Runi))   //%
		gate = eh->dst;
	else
	  if(r->_routetree.type & (Rbcast|Rmulti)) {   //%
		gate = eh->dst;
		sr = v4lookup(f, eh->src, nil);
		if(sr != nil && (sr->_routetree.type & Runi))  //%
		        ifc = sr->_routetree.ifc;              //%
	}
	else
		gate = r->v4.gate;
	...
	if(!canrlock(&ifc->_rwlock))  //% ifc
		goto free;
	if(WASERROR()){  //%
	_ERR_1:
		runlock(&ifc->_rwlock); //% ifc
...
void ipiput4(Fs *f, Ipifc *ifc, Block *bp)
{
	int hl;
	...
	if(notforme) {
		...
		if(r->_routetree.ifc == nil) panic("nil route rfc");   //%
		if(r->_routetree.ifc->reassemble){                     //%
			frag = nhgets(h->frag);
			if(frag) {
				h->tos = 0;
				if(frag & IP_MF)
...
int ipstats(Fs *f, char *buf, int len)
{
	IP *ip;
	char *p, *e;
	int i;
...
Block* ip4reassemble(IP *ip, int offset, Block *bp, Ip4hdr *ih)
{
	int fend;
	ushort id;
	Fragment4 *f, *fnext;
...
void ipfragfree4(IP *ip, Fragment4 *frag)
{
	Fragment4 *fl, **l;
