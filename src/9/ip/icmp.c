...
static void
icmpiput(Proto *icmp, Ipifc* _x, Block *bp)
{
	int	n, iplen;
	Icmp	*p;
	Block	*r;
...
check_clock('g', "icmpiput-1"); //%%
		r = mkechoreply(bp);
		ipriv->out[EchoReply]++;
		ipoput4(icmp->f, r, 0, MAXTTL, DFLTTOS, nil);
check_clock('g', "icmpiput-2\n"); //%%
		break;
	case Unreachable:
		if(p->code > 5)
			msg = unreachcode[1];
