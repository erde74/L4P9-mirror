...
Chan* chandial(char *dest, char *local, char *dir, Chan **ctlp) //% nil ONERR
{
	DS ds;
	char clone[Maxpath];
...
static Chan* call(char *clone, char *dest, DS *ds)  //% nil ONERR
{
	int n;
	Chan *dchan, *cchan;
	...
	*p = 0;
	...
	n = devtab[cchan->type]->write(cchan, name, strlen(name), 0);
	if (n == ONERR) goto _ERR_1;  //% added
