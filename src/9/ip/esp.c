...
espconnect(Conv *c, char **argv, int argc)
{
	...
	case 2:
		...
		*p++ = 0;
		...
		ecb->seq = 0;
		if(strcmp(p, "*") == 0) {
			qlock(&c->p->_qlock);			//% _qlock
			for(;;) {
				spi = nrand(1<<16) + 256;
				if(convlookup(c->p, spi) == nil)
					break;
			}
			qunlock(&c->p->_qlock);			//% _qlock
			ecb->spi = spi;
			ecb->incoming = 1;
			qhangup(c->wq, nil);
		} else {
...
void
espiput(Proto *esp, Ipifc* _x, Block *bp)
{
	Esphdr *eh;
	Esptail *et;
	Userhdr *uh;
	...
	qlock(&esp->_qlock);			//% _qlock
	/* Look for a conversation structure for this port */
	c = convlookup(esp, spi);
	if(c == nil) {
		qunlock(&esp->_qlock);			//% _qlock
...
static int
nullcipher(Espcb* _x, uchar* _y, int _z)
{
	return 1;
}

static void
nullespinit(Espcb *ecb, char *name, uchar* _x, int _y)
{
	ecb->espalg = name;
	ecb->espblklen = 1;
	ecb->espivlen = 0;
...
static int
nullauth(Espcb* _w, uchar* _x, int _y, uchar* _z)
{
	return 1;
}

static void
nullahinit(Espcb *ecb, char *name, uchar* _y, int _z)
{
	ecb->ahalg = name;
	ecb->ahblklen = 1;
	ecb->ahlen = 0;
