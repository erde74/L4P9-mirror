...
Medium* ipfindmedium(char *name)
{
	Medium **mp;

DBGPRN("> ipfindmedium(\"%s\")\n", name);
...
DBGPRN("< ipfindmedium() return=0x%08x\n", *mp);
	return *mp;
}
...
static char* ipifcbind(Conv *c, char **argv, int argc)  //% nil ONERR
{
DBGPRN(">%s(%x %s)\n", __FUNCTION__, _dbg_names(argv, argc));
	Ipifc *ifc;
	Medium *m;
	...
	wlock(&ifc->_rwlock);			//% _rwlock
	if(ifc->m != nil){
		wunlock(&ifc->_rwlock);		//% _rwlock
...
static char* ipifcunbind(Ipifc *ifc)  //% nil ONERR
{
	char *err;
...
static int ipifcstate(Conv *c, char *state, int n)
{
	Ipifc *ifc;
	Iplifc *lifc;
	int m;
...
static int ipifclocal(Conv *c, char *state, int n)
{
	Ipifc *ifc;
	Iplifc *lifc;
	Iplink *link;
...
static void ipifccreate(Conv *c)
{
	Ipifc *ifc;
...
char* ipifcsetmtu(Ipifc *ifc, char **argv, int argc)
{
	int mtu;
...
char* ipifcadd(Ipifc *ifc, char **argv, int argc, int tentative, Iplifc *lifcp)
{
...
static char* ipifcremlifc(Ipifc *ifc, Iplifc *lifc)
{
	Iplifc **l;
	Fs *f;
...
char* ipifcrem(Ipifc *ifc, char **argv, int argc)
{
	uchar ip[IPaddrlen];
...
void ipifcaddroute(Fs *f, int vers, uchar *addr, uchar *mask, uchar *gate, int type)
{
	Medium *m;
	Conv **cp, **e;
	Ipifc *ifc;
...
void ipifcremroute(Fs *f, int vers, uchar *addr, uchar *mask)
{
	Medium *m;
	Conv **cp, **e;
	Ipifc *ifc;
...
static char* ipifcconnect(Conv* c, char **argv, int argc) //% nil ONERR
{
	char *err;
	Ipifc *ifc;
...
char* ipifcsetpar6(Ipifc *ifc, char **argv, int argc)
{
...
char* ipifcsendra6(Ipifc *ifc, char **argv, int argc)
{
	int i;
...
char* ipifcrecvra6(Ipifc *ifc, char **argv, int argc)
{
	int i;
...
static char* ipifcctl(Conv* c, char**argv, int argc)
{
DBGPRN(">%s(%s)\n", __FUNCTION__, _dbg_names(argv, argc));
	Ipifc *ifc;
	int i;
	...
	if(strcmp(argv[0], "add") == 0){
		...
		//% Example use: add 192.168.0.1 255.255.255.0
	}
...
int ipifcstats(Proto *ipifc, char *buf, int len)
{
...
static void iplinkfree(Iplink *p)
{
	Iplink **l, *np;
	ulong now = NOW;
...
static void ipselffree(Ipself *p)
{
	Ipself **l, *np;
	ulong now = NOW;
...
long ipselftabread(Fs *f, char *cp, ulong offset, int n)
{
	int i, m, nifc, off;
	Ipself *p;
	Iplink *link;
...
int iptentative(Fs *f, uchar *addr)
{
 	Ipself *p;
...
Ipifc* findipifc(Fs *f, uchar *remote, int type)
{
	Ipifc *ifc, *x;
	Iplifc *lifc;
	Conv **cp, **e;
...
void findlocalip(Fs *f, uchar *local, uchar *remote)
{
	Ipifc *ifc;
	Iplifc *lifc;
	Route *r;
...
//	USED(atype);					//% USED
//	USED(atypel);					//% USED
	qlock(&f->ipifc->_qlock);			//% _qlock
	r = v6lookup(f, remote, nil);
 	version = (memcmp(remote, v4prefix, IPv4off) == 0) ? V4 : V6;

	if(r != nil){
	        ifc = r->_routetree.ifc;      //%
		if(r->_routetree.type & Rv4)  //%
...
int ipv4local(Ipifc *ifc, uchar *addr)
{
	Iplifc *lifc;
...
int ipv6local(Ipifc *ifc, uchar *addr)
{
	Iplifc *lifc;
...
int ipv6anylocal(Ipifc *ifc, uchar *addr)
{
	Iplifc *lifc;
...
int ipproxyifc(Fs *f, Ipifc *ifc, uchar *ip)
{
	Route *r;
	uchar net[IPaddrlen];
	Iplifc *lifc;
	...
	if(r == nil)
		return 0;
	if((r->_routetree.type & (Rifc|Rproxy)) != (Rifc|Rproxy))  //%
		return 0;
...
int ipifcaddmulti(Conv *c, uchar *ma, uchar *ia) //% ONERR <- void
{
	Ipifc *ifc;
	Iplifc *lifc;
	Conv **p;
...
int ipifcremmulti(Conv *c, uchar *ma, uchar *ia)  //% ONERR <- void
{
	Ipmulti *multi, **l;
	Iplifc *lifc;
	Conv **p;
	...
	*l = multi->next;
	...
	for(p = f->ipifc->conv; *p; p++){
		...
		if(WASERROR()){
			wunlock(&ifc->_rwlock);			//% _rwlock
			NEXTERROR_RETURN(ONERR);  //% Thanks to YU
		}
		wlock(&ifc->_rwlock);				//% _rwlock
		for(lifc = ifc->lifc; lifc; lifc = lifc->next)
			if(ipcmp(ia, lifc->local) == 0)
...
static void ipifcregisterproxy(Fs *f, Ipifc *ifc, uchar *ip)
{
	Conv **cp, **e;
	Ipifc *nifc;
	Iplifc *lifc;
	...
	if(!isv4(ip)) { // V6
		for(cp = f->ipifc->conv; cp < e; cp++){
			...
			rlock(&nifc->_rwlock);			//% _rwlock
			m = nifc->m;
			if(m == nil || m->addmulti == nil) {
				runlock(&nifc->_rwlock);	//% _rwlock
				continue;
			}
...
static void adddefroute6(Fs *f, uchar *gate, int force)
{
	Route *r;

	r = v6lookup(f, v6Unspecified, nil);
	if(r!=nil)
	if(!(force) && (strcmp(r->_routetree.tag,"ra")!=0))	// route entries generated  //%
...
char* ipifcaddpref6(Ipifc *ifc, char**argv, int argc)
{
	uchar	onlink = 1;
	uchar	autoflag = 1;
