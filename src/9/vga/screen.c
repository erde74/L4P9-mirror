...
screensize(int x, int y, int z, ulong chan)
{
	 ...
	 */
	if(scr->vaddr == 0){		// HK 20090930 paddr -> vaddr
		int width = (x*z)/BI2WD;
	...
	/*		memset(gscreendata.bdata, 0x72, width*BY2WD*y);	--- not really black */
		scr->useflush = 1;
		scr->vaddr = VGAMEM();		// HK 20090930 paddr -> vaddr
		scr->vaddr = KADDR(scr->vaddr);	// HK 20090930 paddr -> vaddr
		scr->apsize = 1<<16;
	}
	else
...
//	if(scr->vaddr)	/* set up during enable */	// HK 20090930 paddr -> vaddr
//		return 0;
...
void vgalinearaddr(VGAscr *scr, ulong laddr, int size)
{
	int x, nsize;
	ulong npaddr;
