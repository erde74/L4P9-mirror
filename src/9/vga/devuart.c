...
uartenable(Uart *p)
{
	...
	lock(&uartalloc._lock);	// HK 20091031
	...
	p->enabled = 1;
	unlock(&uartalloc._lock);	// HK 20091031
...
{
	qlock(&p->_qlock);	// HK 20091031
	if(p->opens++ == 0 && uartenable(p) == nil){
		qunlock(&p->_qlock);	// HK 20091031
		error(Enodev);
	}
	if(setb1200)
		uartctl(p, "b1200");
...
static long
uartwrite(Chan *c, void *buf, long n, vlong dummy)	// HK 20091031
{
	Uart *p;
	char *cmd;
