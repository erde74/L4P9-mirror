...
static void
vmwarelinear(VGAscr* scr, int _x, int _y)  //%
{
	char err[64];
	Pcidev *p;
...
static void
vmwareflush(VGAscr* _x, Rectangle r)  //%
{
	if(vm->mmio == nil)
		return;
...
static int
vmwarescroll(VGAscr* _x, Rectangle r, Rectangle sr) //%
{
	if(vm->mmio == nil)
		return 0;
...
static int
vmwarefill(VGAscr* _x, Rectangle r, ulong sval) //%
{
	if(vm->mmio == nil)
		return 0;
