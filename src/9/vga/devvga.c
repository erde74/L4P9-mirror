...
static Dirtab vgadir[] = {
  {".",	{ Qdir, 0, QTDIR },		0,	0550},   //%
	...
	{ "vgadrvctl", { Qvgadrvctl, 0 }, 0, 0660 },	// HK 20090831
};
...
char vga_bios_map = 0;	// HK 20091031
...
vgareset(void)
{
	...
	if (vga_bios_map == 0) {
		if (lp49_mappages(0x60000, BIOS_LADDR + 0xa0000, 0xa0000))
			panic("devvga.c: vga bios mapping error");
		vga_bios_map = 1;
	}
	return;
	// HK 20091031 end
...
vgaread(Chan* c, void* a, long n, vlong off)
{
	...
	case Qvgabios:
		if(offset >= 0x100000)
			return 0;
		if (offset < 0x000a0000)	// HK 20090930
			return 0;		// HK 20090930
		if(offset+n >= 0x100000)
			n = 0x100000 - offset;
		memmove(a, (uchar*)/* kaddr(0)+ */ BIOS_LADDR + offset, n);	// HK 20090831
		return n;
	...
	case Qvgactl:
		...
		len += snprint(p+len, READSTR-len, "addr p 0x%lux v 0x%p size 0x%ux\n",
			scr->vaddr, scr->vaddr, scr->apsize);	// HK 20090930 paddr -> vaddr
		//%  USED(len);
...
vgactl(Cmdbuf *cb)
{
	...
	case CMhwgc:
		if(strcmp(cb->f[1], "off") == 0){
			lock(&cursor._lock);  //% lock(&cursor);
			if(scr->cur){
				if(scr->cur->disable)
					scr->cur->disable(scr);
				scr->cur = nil;
			}
			unlock(&cursor._lock);  //% lock(&cursor);
			return;
		}
	...
	case CMdrawinit:
		if(scr && scr->dev && scr->dev->drawinit)
			scr->dev->drawinit(scr);
		memimagedraw(scr->gscreen, scr->gscreen->r, memblack, ZP, nil, ZP, S);	// HK 20091031, debug
		return;
