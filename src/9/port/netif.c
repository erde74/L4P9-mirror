 ...
 */
static int
netifgen(Chan *c, char* _x, Dirtab *vp, int _y, int i, Dir *dp)
{
	Qid q;
	Netif *nif = (Netif*)vp;
	Netfile *f;
...
Chan*
netifopen(Netif *nif, Chan *c, int omode)  //% ONERR
{
	int id;
	Netfile *f;
...
long
netifread(Netif *nif, Chan *c, void *a, long n, ulong offset)  //% ONERR
{
	int i, j;
	Netfile *f;
	char *p;
 ...
 */
long
netifwrite(Netif *nif, Chan *c, void *a, long n)  //% ONERR
{
	Netfile *f;
	int type;
	char *p, buf[64];
	...
	if(WASERROR()){  //%
	_ERR_1:
	        qunlock(&nif->_qlock);    //% qunlock(nif);  
		NEXTERROR_RETURN(ONERR);  //%
	}

	qlock(&nif->_qlock);         //%  qlock(nif);
	f = nif->f[NETID(c->qid.path)];
...
int
netifwstat(Netif *nif, Chan *c, uchar *db, int n)  //% ONERR
{
	Dir *dir;
	Netfile *f;
	int m;
