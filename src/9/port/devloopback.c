...
loopbackattach(char *spec)
{
	...
	qlock(&lb->_qlock);  //% qlock(lb);
	if(waserror()){
		lb->ref--;
		qunlock(&lb->_qlock); //% lock(lb);
		nexterror();
	}
	...
	c->_u.aux = lb;  //% c->aux = lb;
	c->dev = dev;
	return c;
}

static int
loopbackgen(Chan *c, char* _x, Dirtab* _y, int _z, int i, Dir *dp)  //%
{
	Dirtab *tab;
	int len, type;
	Qid qid;
...
loopbackwalk(Chan *c, Chan *nc, char **name, int nname)
{
	...
	if(wq != nil && wq->clone != nil && wq->clone != c){
	        lb = c->_u.aux;  //% lb = c->aux;
		qlock(&lb->_qlock); //% lock(lb);
		lb->ref++;
...
closelink(Link *link, int dofree)
{
	...
	link->tin = 0;
	timerdel(&link->ci);
	iunlock(&link->_lock); //% lock(link);
	if(iq != nil){
		qclose(iq);
		if(dofree){
			ilock(&link->_lock); //% lock(link);
			free(iq);
			link->iq = nil;
			iunlock(&link->_lock); //% lock(link);
		}
