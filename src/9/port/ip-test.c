...
//  Block *bp;
  ...
  DBGPRN("<><><><><> PROTOCOL STACK TEST START <><><><>\n");
...
//  chandevinit();
  ...
  DBGBRK("====== open(#I/ndb) ======\n");
  cp1 = ipdevtab.open(cp1, OREAD);
  DBGPRN("rc = %p\n", cp1);

  DBGBRK("====== read(#I/ndb) ======\n");
  rc = ipdevtab.read(cp1, buf, sizeof(buf), 0);
  DBGPRN("rc = %p\n", rc);
  DBGPRN("%s\n", buf);
  ...
  cp3 = ipdevtab.open(cp3, OWRITE);
  DBGPRN("rc = 0x%08x\n", cp3);

#define CONTYPE "connect 2054"

  DBGBRK("====== Write(#l/ether0/0/ctl, \"%s\") ======\n", CONTYPE);
  rc = ipdevtab.write(cp3, CONTYPE, sizeof(CONTYPE) - 1, 0);
  DBGPRN("rc = %d\n", rc);
  ...
  cp2 = ipdevtab.open(cp2, ORDWR);

  /***************************************************************************/
  /* Transmit and Receive section											 */
  /***************************************************************************/
  DBGBRK("===== TRANSMIT =====\n");
  rc = ipdevtab.write(cp2, &data, data.size, 0);
  DBGPRN("write rc=%d\n", rc);


  DBGBRK("===== RECEIVE  =====\n");
//  while(1){
    rc = ipdevtab.read(cp2, buf, sizeof(buf), 0);
    hexdump(buf, rc);
	DBGBRK("Any key ...\n");
//  }
