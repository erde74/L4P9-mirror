...
#define   _DBGFLG  0
#include  <l4/_dbgpr.h>
#define   XXX(s)  // if((uint)(&q->rlock.use.key)<=0x1400078) l4printf_r("%s:%s:qlock<%x:%x>  ", __FUNCTION__, s, &q->rlock.use, q->rlock.use.key)
 ...
 *  return nil on error.
 */
Block* mem2bl(uchar *p, int len) //% ONERR
{
	int n;
	Block *b, *first, **l;
	...
	_ERR_1:
		freeblist(first);
		NEXTERROR_RETURN(nil);  //% Nnexterror();
	}
	do {
		n = len;
		if(n > Maxatomic)
 ...
 *  called with q ilocked
 */
static void qwakeup_iunlock(Queue *q)
{
	int dowakeup = 0;
 ...
 */
Block* qbread(Queue *q, int len)  //% ONERR
{
	Block *b, *nb;
	int n;
	...
	case 0:
		...
		qunlock(&q->rlock); 
		POPERROR();  //% 
		l4printf_c(12, "qbread-Er2 ");  //%
		return nil;
	case -1:
		/* multiple reads on a closed queue */
		iunlock(&q->_lock);   //% iunlock(q);
		l4printf_c(12, "qbread-Er3 ");  //%
		ERROR_GOTO(q->err, _ERR_1);  //%  error(q->err);
	}
...
{  int i, j; char ch;
   ...
   for (i=0; i<rc; i++){
   if (ss[i] == 'T') {
     l4printf_r(" qread() ss[%d]==T <- %x :", i, up->thread.raw);
     for (j=0; j<rc && j <80; j++)
       if ((ch=ss[j]) >= ' ' && ch <= '}' )
	 l4printf_r("%c",ch);
     l4printf_r("\n");
     
          dump_mem("T", ss, (rc<128)?rc:128);
     //     _backtrace_();
     return;
   }
 }
 ...
 */
long qread(Queue *q, void *vp, int len)  //% ONERR
{
	Block *b, *first, **l;
	int m, n;
...
again:
	...
	case 0:
		...
		qunlock(&q->rlock);
		POPERROR();   //%
		l4printf_c(12, "qread-Er2 ");  //%		
		DBGPRN("< %s()NG->0 \n", __FUNCTION__);
		return 0;
	case -1:
		/* multiple reads on a closed queue */
		iunlock(&q->_lock);   //% iunlock(q);
		l4printf_c(12, "qread-Er3 ");  //%
		ERROR_GOTO(q->err, _ERR_1);  //%   error(q->err);
	}
	...
	//% if (_DBGFLG) dump_mem(__FUNCTION__, vp, len); 
	DBGPRN("< %s()OK->%d \n", __FUNCTION__, n);
	// _DYY_(vp, n);   //%%%
	return n;
}
 ...
 */
long  qbwrite(Queue *q, Block *b) //% ONERR 
{
	int n, dowakeup;
	Proc *p;
 ...
 */
int qwrite(Queue *q, void *vp, int len)  //% ONERR
{
	int n, sofar;
	Block *b;
	uchar *p = vp;
