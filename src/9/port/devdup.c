...
#if 0  //%% DUMMY %%%%%%%%%%%%%%%
int procfdprint(Chan *c, int fd, int w, char *s, int ns) // devproc.c
{  }
#endif //-------------------

static int
dupgen(Chan *c, char * _x, Dirtab* _y, int _z, int s, Dir *dp)
{
	Fgrp *fgrp = up->fgrp;
	Chan *f;
...
static long
dupwrite(Chan* _w, void* _x, long _y, vlong _z)
{
	panic("dupwrite");
