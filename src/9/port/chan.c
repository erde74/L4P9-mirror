//%%%% Very complexted program: hoped to be refined. %%%%%
...
#define   XX   if(_DBGFLG2)l4printgetc 

extern int l4printf(const char*, ...);
extern int l4printf_b(const char*, ...);
extern int l4printf_r(const char*, ...);
extern int l4printf_g(const char*, ...);

extern void pr_chan(Chan*);
void pr_ch(Chan *cp,  char *msg)
{
    PRN("%s", msg);   pr_chan(cp);
}

//%----- debug-dump-only; Serial-use-only ------
...
/* static void */ int   //% called freom _dumptbl.c 
dumpmount(void)		/* DEBUGGING */
{
	...
	for(h = pg->mnthash; h < he; h++){
		for(f = *h; f; f = f->hash){
			print("head: %p: %s 0x%x.%d %c %d -> \n", f,
				f->from->path->s, (uint)f->from->qid.path,
				f->from->qid.vers, devtab[f->from->type]->dc,
				f->from->dev);
			for(t = f->mount; t; t = t->next)
				print("\t%p: %s (umh %p) (path %x dev %c %d)\n", t, t->to->path->s, t->to->umh, (uint)t->to->qid.path, devtab[t->to->type]->dc, t->to->dev);
		}
	}
	POPERROR();
	runlock( &pg->ns ); //%
...
void chandevreset(void)
{
	...
	for(i=0; devtab[i] != nil; i++){
	        if (get_new_tid_and_utcb(0, 0) == -1)  //!!!
		  l4printf_b("chandevreset-1<%s %x> ", devtab[i]->name, devtab[i]->reset);

		devtab[i]->reset();

	        if (get_new_tid_and_utcb(0, 0) == -1)  //!!!
		  l4printf_b("chandevreset-2<%s %x> ", devtab[i]->name, devtab[i]->reset);

		// memory interact in {mnt,ether,ip,sd}reset() ?
	}
}
...
Chan* newchan(void)
{
	...
	lock(&chanalloc._lock );  //%
	c = chanalloc.free;
	if(c != 0)
		chanalloc.free = c->next;
	unlock(&chanalloc._lock ); //%
...
static Path* copypath(Path *p)
{
	...
	for(i=0; i<pp->mlen; i++){
		pp->mtpt[i] = p->mtpt[i];
		if(pp->mtpt[i])
		  incref( &pp->mtpt[i]->_ref);  //%
	}

	return pp;
}

int pathclose(Path *p)   //% ONERR <- void
{
	int i;
	...
	if(p == nil)
	        return  1;  //%  return;
//XXX
	DBG("pathclose %p %s ref=%ld =>", p, p->s, p->_ref.ref); //%
	for(i=0; i<p->mlen; i++)
		DBG(" %p", p->mtpt[i]);
	DBG("\n");
...
static Path* uniquepath(Path *p)   //% ONERRNIL
{
	Path *new;
...
static Path* addelem(Path *p, char *s, Chan *from)   //% ONERR
{
	char *t;
	int a, i;
	Chan *c, **tt;
...
int cclose(Chan *c)   //% ONERR <- void
{
        int  rc; //%
	if(c->flag&CFREE)
		panic("cclose %lux", getcallerpc(&c));

	DBG("cclose %p name=%s ref=%ld\n", c, c->path->s, c->_ref.ref); //%
	if(decref( &c->_ref) )  //%
	        return  0;  //%

	if(!WASERROR()){  //%
	        /* rc =   ???????? */
	        devtab[c->type]->close(c); //% ONERR
...
static int clunkwork(void* _x)
{
	return clunkq.head != nil;
}

void closeproc(void* _x)     //% ONERR ?
{
	Chan *c;
	int  rc;
	...
	for(;;){
	  qlock( &clunkq.q);  //%
		if(clunkq.head == nil){
			if(!WASERROR()){
			        rc = tsleep(&clunkq.r, clunkwork, nil, 5000); //%
				IF_ERR_GOTO(rc, ONERR, _ERR_1); //%
				POPERROR();
			}
...
int cmount(Chan **newp, Chan *old, int flag, char *spec)  //% ONERR
{
	...
	Mount *nm, *f, *um, **h;
	Pgrp *pg;
	Chan  *newin = *newp; //DEBUG

	DBGBRK("> %s(\"%s\" \"%s\" %x spec;\"%s\")\n",
	       __FUNCTION__, (*newp)->path->s, old->path->s, flag, spec);

	if(QTDIR & (old->qid.type^(*newp)->qid.type))
	        ERROR_RETURN(Emount, ONERR);  //% 
	...
	DBGPRN("< %s( )-> %d \n", __FUNCTION__, nm->mountid);
DBGPRN("<cmount{new:%s old:%s}\n", new->path->s, old->path->s);
	return nm->mountid;
}


int cunmount(Chan *mnt, Chan *mounted)  //% ONERR <- void
{
	Pgrp *pg;
	Mhead *m, **l;
	Mount *f, **p;
	...
	PRN(">unmount(%s, %s)\n", 
	    (mnt)?mnt->path->s:"-", (mounted)?mounted->path->s:"-");
	...
	wunlock(&m->lock);
	wunlock(&pg->ns);
	ERROR_RETURN(Eunion, ONERR);	//% 
}


Chan* cclone(Chan *c)  //% ONERRNIL
{
	Chan *nc;
	Walkqid *wq;


	//PRN("cclone1('%s',%c) ", c->path->s, devtab[c->type]->dc);	//%%%%%%
	wq = devtab[c->type]->walk(c, nil, nil, 0);

	//PRN("cclone2(%s) ", wq->clone->path->s);	// pr_chan(c); //%%%%%%


	if(wq == nil) {
	        ERROR_RETURN("clone failed", nil);  //% 
	}
	nc = wq->clone;
...
int findmount(Chan **cp, Mhead **mp, int type, int dev, Qid qid)
{
	...
	DBGPRN(">%s(tdq<%d %d %d>)  ", __FUNCTION__, type, dev, (ulong)qid.path);
	pg = up->pgrp;
	rlock(&pg->ns);
	...
	for(m = MOUNTH(pg, qid); m; m = m->hash){
		...
		if(eqchantdqid(m->from, type, dev, qid, 1)){
			...
			if(*cp != nil)
				cclose(*cp);
			incref( &m->mount->to->_ref );  //%
			*cp = m->mount->to;
			// pr_chan(*cp);  //%
			runlock(&m->lock);

			DBGPRN("<%s(C:\"%s\" mp->fromC:\"%s\")\n", 
			       __FUNCTION__,  (*cp)->path->s, 
			       (mp)?(*mp)->from->path->s:"-"); 
 ...
 */
static int domount(Chan **cp, Mhead **mp, Path **path)  //% ONERR
{
	Chan **lc;
	Path *p;

	DBGPRN("> %s(*cp:%d(%s) *mp:%d(%s) *path:%s)\n", 
	       __FUNCTION__, (*cp)->fid, (*cp)->path->s, 
	       (mp==0)?0:(*mp)->from->fid, (mp==0)?"-":(*mp)->from->path->s,  (*path)->s); 

	if(findmount(cp, mp, (*cp)->type, (*cp)->dev, (*cp)->qid) == 0) {
	        DBGPRN("< %s(c:%d(%s) %x %x)=>NoMnt\n", __FUNCTION__, 
		       (*cp)->fid, (*cp)->path->s,  (*mp), (*path));  //%
		return 0;
	}
...
DBG("domount %p %s => add %p (was %p)\n", p, p->s, (*mp)->from, p->mtpt[p->mlen-1]);
	...
	DBGPRN("< %s(c:%d(%s) fromC:%d(%s) %s)=>Mnt\n", __FUNCTION__, 
	       (*cp)->fid, (*cp)->path->s,  (*mp)->from->fid, (*mp)->from->path->s, (*path)->s);  //%
	return 1;
}
/*                               ---> +- Chan -+ <-------- *cp:input
 *                              /     |        |  EQ(type,dev,qid)      
 ...
 */
static Chan* undomount(Chan *c, Path *path)  //% NIL: ERR
{
	Chan *nc;

	if(path->_ref.ref != 1 || path->mlen == 0)  //%
		print("undomount: path %s ref %ld mlen %d caller %lux\n",
		      path->s, path->_ref.ref, path->mlen, getcallerpc(&c)); //%
...

static char *wqidlist(Walkqid *wp)  // Non-reentrant
{
  static  char ebuf[64], fbuf[16];
  int  i;

  l4snprintf(ebuf, 64, "walkqid{nqid=%d ",  wp->nqid);

  for(i=0; i<wp->nqid; i++){
     l4snprintf(fbuf, 16, "<%d-%d>", wp->qid[i].type, (ulong)wp->qid[i].path);
     strcat(ebuf, fbuf);   strcat(ebuf, ",");
  }
  strcat(ebuf, "}");
  return ebuf;
}
 ...
 */
static Walkqid* ewalk(Chan *c, Chan *nc, char **name, int nname)   //% ONERRNIL
{
	Walkqid *wq;

	DBGPRN("> ewalk(c:%d(%s) nc:%d(%s) names{%s})\n",  
		c->fid, c->path->s, (nc)?nc->fid:0, (nc)?nc->path->s:"-", _dbg_names(name, nname));
...
if(0){//DEBUG
  int  i;
  if (wq == nil){  l4printf_g("ewalk(c:'%s' nc:'%s' ", c->path->s, nc->path->s) ;
    for(i=0; i<nname; i++)l4printf_g(" '%s' ", name[i]);
    l4printf_g(")\n");
  }
}
	IF_ERR_GOTO(wq, nil, _ERR_1);  //% can be optimized.

	POPERROR();

	DBGPRN("< ewalk c:%d(%s) => nc:%d(%s) clone:%d(%s) %s\n", 
	     c->fid, c->path->s, (nc)?nc->fid:0, (nc)?nc->path->s:"-", 
	     (wq)?wq->clone->fid:0, (wq)?wq->clone->path->s:"-", wqidlist(wq));//%
...
DBGPRN("domount(c:'%s' mh:'%s' path:'%s'\n", c->path->s,  mh->from->path->s, path->s);
...
DBGPRN("EWALK1(%s %s)\n", c->path->s, _dbg_names(names+nhave, ntry));
			if(mh && !nomount){
				/*
				...
				for(f = mh->mount->next; f; f = f->next){
				        wq = ewalk(f->to, nil, names+nhave, ntry);

DBGPRN(" EWALK2(%s %s){from:'%s' to:'%s' wq:%s}\n", 
    f->to->path->s, _dbg_names(names+nhave, ntry),
    mh->from->path->s, f->to->path->s, (wq)?wq->clone->path->s:"nil");
 ...
 */
Chan* createdir(Chan *c, Mhead *m)  //% ONERR NIL
{
	Chan *nc;
	Mount *f;
...
int namelenerror(char *aname, int len, char *err)  //% ONERR: <- void
{
	char *ename, *name, *next;
	int i, errlen;
	// l4printf_c(7, " >namelenerror('%s' '%s') ", aname, err);//%
	/*
	...
	snprint(up->errstr, ERRMAX, "%s; %s", up->genbuf, err);
	NEXTERROR_RETURN(ONERR);   //% 
}

int nameerror(char *name, char *err)   //% ONERR <- void
{
        return namelenerror(name, strlen(name), err); //%
}
...
Chan* namec(char *aname, int amode, int omode, ulong perm)  //% ONERRNIL
{
	int len, n, t, nomount;
	Chan *c, *cnew;
	Path *path;
...
DBGPRN(">%s(\"%s\", %x, %x, %x) \n", __FUNCTION__, aname, amode, omode, perm);

	if(aname[0] == '\0')
	        ERROR_RETURN("empty file name", nil);  //% 
	...
	case '/':
		...
		c = up->slash;
//if(strcmp(aname, "/t/b/ex2")==0)print("chan=%s", c->path->s);
	...
	_ERR_1_1:
		...
		free(e.off);
		rc = namelenerror(aname, len, tmperrbuf); //%

//L4_KDB_Enter("namec-10");
		IF_ERR_GOTO(rc, ONERR, _ERR_1);
	}
	// DBGBRK("? %s  20\n", __FUNCTION__); 
	...
	if(amode == Acreate){
		...
		if(e.mustbedir && !(perm&DMDIR)){
			e.nerror = e.nelems;
			ERROR_GOTO("create without DMDIR", _ERR_1_1);  //%
		}

		/* don't try to walk the last path element just yet. */
		if(e.nelems == 0)
		        ERROR_GOTO(Eexist, _ERR_1_1); //%

		e.nelems--;
	}

        rcode = walk(&c, e.elems, e.nelems, nomount, &e.nerror);
	if(rcode  < 0){
...
//DEBUG-DUMP
//l4printf_g("c:'%s' elems:'%s' n:%d nomnt:%d ne:%d\n",
//	   c->path->s, (e.nelems)?e.elems[e.nelems-1]:"", e.nelems, nomount, e.nerror);
//L4_KDB_Enter("namec-30");

		NEXTERROR_GOTO(_ERR_1_1);  //% 
	}  


	if(e.mustbedir && !(c->qid.type&QTDIR))
	        ERROR_GOTO("not a directory", _ERR_1_1); //%

	if(amode == Aopen && (omode&3) == OEXEC && (c->qid.type&QTDIR))
	        ERROR_GOTO("cannot exec directory", _ERR_1_1); //%
...
if(c==nil)L4_KDB_Enter("namec-20");
			IF_ERR_GOTO(c, nil, _ERR_1_1); //% ?????

			// DBGPRN("$60(c:%d, type=%d path-s=%s devname=%s) \n", 
			//       c->fid, c->type, c->path->s, devtab[c->type]->name);
	...
	case Acreate:
		...
		if(!WASERROR()){	/* try create */
			...
			pathclose(cnew->path);
			cnew->path = c->path;
			incref( &cnew->path->_ref );  //%

			// DBG_CH(cnew, __FUNCTION__ " 67");  //%%%%%

			rc = devtab[cnew->type]->create(cnew, e.elems[e.nelems-1], omode&~(OEXCL|OCEXEC), perm);
			IF_ERR_GOTO(rc, nil, _ERR_3); //% ?????
	...
	_ERR_3:
		...
		if(walk(&c, e.elems+e.nelems-1, 1, nomount, nil) < 0){
			up->errstr = createerr;
			ERROR_GOTO(createerr, _ERR_1_1);  /* report true error */  //%
		}
	...
	free(e.elems);
	free(e.off);
	POPERROR();	/* e c  _ERRE_1_1 */

	free(aname);
	POPERROR();	/* aname  _ERR_1 */

	// pr_chan(c); //%
	DBGBRK("< namec()=> chan:%d(%s)\n", c->fid, c->path->s);
	return c;
}
...
static char* validname0(char *aname, int slashok, int dup, ulong pc)   // nil : ERR
{
	char *p, *ename, *name, *s;
	uint t;
	int c, n;
	...
	DBGPRN("> validname0(%s %d %d %x)\n", aname, slashok, dup, pc);
	name = aname;
	...
	if(ename==nil || ename-name>=(1<<16))
	        ERROR_RETURN("name too long", nil);  //%
...
int isdir(Chan *c)  //% ONERR <- void
{
	if(c->qid.type & QTDIR)
		return 1;
	
	DPG("Chan<%d> %x \"%s\"", c->fid, c->qid.type, c->path->s);  //%
	return  ONERR;  //% 
}
