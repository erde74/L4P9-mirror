...
//%  typedef struct Confmem	Confmem;   //! LP49
...
#if  THREADQUE //%-------------------
typedef  struct Threadque   Threadque;
struct Threadque
{
        L4_ThreadId_t  tid;
        int            state;
...
struct RWlock
{
	Lock	use;
#if THREADQUE
	Threadque	*head;		/* list of waiting thread */
	Threadque	*tail;
#else //original
...
struct Chan
{
        Ref    _ref; //% /* the Lock in this Ref is also Chan's lock */
...
struct Dev
{
	...
	Chan*	(*open)(Chan*, int);
        ...
        //%orig.   void	(*create)(Chan*, char*, int, ulong);  
        int	(*close)(Chan*);                    //% ONERR <- void
        //%orig.   void	(*close)(Chan*);
	long	(*read)(Chan*, void*, long, vlong);
	Block*	(*bread)(Chan*, long, ulong);
	long	(*write)(Chan*, void*, long, vlong);
	long	(*bwrite)(Chan*, Block*, ulong);
        int	(*remove)(Chan*);    //% ONERR <- void
        //%orig.   void	(*remove)(Chan*);
...
struct Pgrp
{
        Ref     _ref; //%	/* also used as a lock when mounting */
	int	noattach;
	ulong	pgrpid;
...
struct Rgrp
{
        Ref     _ref; //%		/* the Ref's lock is also the Rgrp's lock */
...
struct Proc
{
        L4_ThreadId_t  thread;  //% L4 thread of this process
  ...
  //%	uvlong	kentry;		/* Kernel entry time stamp (for profiling) */
	...
	Proc	*rendhash;	/* Hash list for tag values */
        ...
        Timer   _timer;	 //%    /* For tsleep and real-time */
	Rendez	*trend;
	int	(*tfn)(void*);
	void	(*kpfun)(void*);
	void	*kparg;
