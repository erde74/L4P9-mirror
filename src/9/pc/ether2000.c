...
ne2000match(Ether* edev, int id)
{
	...
	for(ctlr = ctlrhead; ctlr != nil; ctlr = ctlr->next){
		...
		if(edev->_isaconf.port != 0 && edev->_isaconf.port != port)
			continue;
		 ...
		 */
		edev->_isaconf.port = port;
		edev->_isaconf.irq = p->intl;
...
ne2000pnp(Ether* edev)
{
	 ...
	 */
	id = 0;
	for(i = 0; i < edev->_isaconf.nopt; i++){
		if(cistrncmp(edev->_isaconf.opt[i], "id=", 3) == 0)
			id = strtol(&edev->_isaconf.opt[i][3], nil, 0);
	}
...
ne2000reset(Ether* edev)
{
	...
	if(edev->_isaconf.port == 0)
		return -1;
	if(edev->_isaconf.irq == 0)
		edev->_isaconf.irq = 2;
	if(edev->_isaconf.mem == 0)
		edev->_isaconf.mem = 0x4000;
	if(edev->_isaconf.size == 0)
		edev->_isaconf.size = 16*1024;
	port = edev->_isaconf.port;

	if(ioalloc(edev->_isaconf.port, 0x20, 0, "ne2000") < 0)
		return -1;
	...
	dp8390->tstart = HOWMANY(edev->_isaconf.mem, Dp8390BufSz);
	dp8390->pstart = dp8390->tstart + HOWMANY(sizeof(Etherpkt), Dp8390BufSz);
	dp8390->pstop = dp8390->tstart + HOWMANY(edev->_isaconf.size, Dp8390BufSz);

	dp8390->dummyrr = 1;
	for(i = 0; i < edev->_isaconf.nopt; i++){
		if(strcmp(edev->_isaconf.opt[i], "nodummyrr"))
			continue;
		dp8390->dummyrr = 0;
		break;
	}
