...
#define BOUNCEBUF_LADDR		(1<<30)		//HK 20070307

int request_alloc_for_dma(unsigned long size, unsigned long logical_addr, unsigned long *physical_addr);	//HK 20070307
...
{
	void* bva;
        unsigned long bouncebuf_paddr;	//HK 20070307
	...
	// bva = xspanalloc(64*1024*i8237dma, BY2PG, 64*1024);  //%			//HK 20070307
	bva = nil;									//HK 20070307
        if(request_alloc_for_dma(64*1024*3, BOUNCEBUF_LADDR, &bouncebuf_paddr) == 0)	//HK 20070307
		bva = (void*)(BOUNCEBUF_LADDR+((-bouncebuf_paddr)&0xffff));		//HK 20070307
...
dmasetup(int chan, void *va, long len, int isread)
{
	 ...
	 */
	ilock(& dp->_lock);  //%  <- ilock(dp);  
	mode = (isread ? 0x44 : 0x48) | chan;
	outb(dp->_dmaport.mode, mode);	/* single mode dma (give CPU a chance at mem) */
	outb(dp->_dmaport.page[chan], pa>>16);  //%
	outb(dp->_dmaport.cbp, 0);	/* set count & address to their first byte */
	outb(dp->_dmaport.addr[chan], pa>>dp->shift);	/* set address */  //%
	outb(dp->_dmaport.addr[chan], pa>>(8+dp->shift));   //%
	outb(dp->_dmaport.count[chan], (len>>dp->shift)-1);	/* set count */ //%
	outb(dp->_dmaport.count[chan], ((len>>dp->shift)-1)>>8); //%
	outb(dp->_dmaport.sbm, chan);	/* enable the channel */  //%
	iunlock(& dp->_lock);  //%  <- iunlock(dp);  //%
...
dmadone(int chan)
{
	...
	return inb(dp->_dmaport.cmd) & (1<<chan);  //% _
}
...
dmaend(int chan)
{
	 ...
	 *  disable the channel
	 */
	ilock(& dp->_lock);   //%   orig: ilock(dp)
	outb(dp->_dmaport.sbm, 4|chan);  //% _
	iunlock(& dp->_lock);  //%  orig: iunlock(dp)
