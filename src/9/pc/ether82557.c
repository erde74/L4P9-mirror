...
ifstat(Ether* ether, void* a, long n, ulong offset)
{
	...
	//	;
        for(timeo = 1;timeo < 1000; timeo++){
	  delay(1);
	}
	if (ctlr->dump[16] ==0)
	  PRN("ifstat:DunmSC command timeout\n");
	
	ether->_netif.oerrs = ctlr->dump[1]+ctlr->dump[2]+ctlr->dump[3];
	ether->_netif.crcs = ctlr->dump[10];
	ether->_netif.frames = ctlr->dump[11];
	ether->_netif.buffs = ctlr->dump[12]+ctlr->dump[15];
	ether->_netif.overflows = ctlr->dump[13];
...
reset(Ether* ether)
{
	...
	for(ctlr = ctlrhead; ctlr != nil; ctlr = ctlr->next){
		if(ctlr->active)
			continue;
		if(ether->_isaconf.port == 0 || ether->_isaconf.port == ctlr->port){
			ctlr->active = 1;
			break;
		}
	}
	 ...
	 */
	ether->ctlr = ctlr;
	ether->_isaconf.port = ctlr->port;
	ether->_isaconf.irq = ctlr->pcidev->intl;
	...
	if(phyaddr >= 0){
		...
		case 0x0A:				/* DP83840A */
			...
			x |= 0x0420;
			for(i = 0; i < ether->_isaconf.nopt; i++){
				if(cistrcmp(ether->_isaconf.opt[i], "congestioncontrol"))
					continue;
				x |= 0x0100;
				break;
			}
		...
		if(anlpar == 0){
			...
			for(i = 0; i < ether->_isaconf.nopt; i++){
				for(k = 0; k < nelem(mediatable); k++){
					if(cistrcmp(mediatable[k], ether->_isaconf.opt[i]))
						continue;
					medium = k;
					break;
				}
	...
	ether->_netif.promiscuous = promiscuous;
	ether->_netif.multicast = multicast;
	ether->_netif.arg = ether;
	//PRN(">%s()\n", __FUNCTION__);
	return 0;
}
