...
static void
vt6102interrupt(Ureg* ureg, void* arg)
{
	Ctlr *ctlr;
	Ether *edev;
	int imr, isr, r, timeo;
...
vt6102pnp(Ether* edev)
{
	...
	for(ctlr = vt6102ctlrhead; ctlr != nil; ctlr = ctlr->next){
		if(ctlr->active)
			continue;
		if(edev->_isaconf.port == 0 || edev->_isaconf.port == ctlr->port){
			ctlr->active = 1;
			break;
		}
	}
	...
	edev->ctlr = ctlr;
	edev->_isaconf.port = ctlr->port;
	edev->_isaconf.irq = ctlr->pcidev->intl;
	edev->tbdf = ctlr->pcidev->tbdf;
	edev->_netif.mbps = 100;
	...
	edev->_netif.arg = edev;
	edev->_netif.promiscuous = vt6102promiscuous;
	edev->_netif.multicast = vt6102multicast;
