...
{
        lock(&cb->_lock);  // lock(cb)
	engine(cb, message);
	unlock(&cb->_lock);   // unlock(cb)
}
...
static int
pccardgen(Chan *c, char* _x, Dirtab * _y, int _z, int i, Dir *dp)  //%
{
	int slotno;
	Qid qid;
	long len;
...
static long
pccardwrite(Chan *c, void *v, long n, vlong _x) //%
{
	Rune r;
	ulong n0;
	char *device;
...
static void
tcfig(Cardbus *cb, Cisdat *cis, int _x)  //%
{
...
static void
tvers1(Cardbus *cb, Cisdat *cis, int _x)  //%
{
...
static void
tentry(Cardbus *cb, Cisdat *cis, int _x)  //%
{
	uchar c, i, feature;
	PCMconftab *ct;
	Pcminfo *pi;
