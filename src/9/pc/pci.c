...
extern  void dbg_pagemap(char *title);
...
static void
pcirouting(void)   //% pages[8f0000-900000) are mapped.
{
	Slot *e;
	Router *r;
	int size, i, fn, tbdf;
...
dbg_pagemap("pcirouting1"); //%
	...
	for(p = (uchar *)KADDR(0xf0000); p < (uchar *)KADDR(0xfffff); p += 16)
		if(p[0] == '$' && p[1] == 'P' && p[2] == 'I' && p[3] == 'R')
			break;
dbg_pagemap("pcirouting2"); //%
        if(p >= (uchar *)KADDR(0xfffff)) { print("pcirouting-skipped\n");
		return;
	}
	r = (Router *)p;
...
static void
pcicfginit(void)    //% pages [8f0000-900000) are mapped.
{
	char *p;
	Pcidev **list;
	ulong mema, ioa;
