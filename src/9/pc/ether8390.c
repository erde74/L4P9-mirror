...
receive(Ether* ether)
{
	...
	for(curr = getcurr(ctlr); ctlr->nxtpkt != curr; curr = getcurr(ctlr)){
		...
		if(ctlr->ram)
			memmove(&hdr, (void*)(ether->_isaconf.mem+data), sizeof(Hdr));
		else
		...
		if((hdr.status & (Fo|Fae|Crce|Prxok)) == Prxok && (bp = iallocb(len))){
			...
			if((data+len) >= ctlr->pstop*Dp8390BufSz){
				...
				if(ctlr->ram)
					memmove(p, (void*)(ether->_isaconf.mem+data), count);
				else
			...
			if(len){
				if(ctlr->ram)
					memmove(p, (void*)(ether->_isaconf.mem+data), len);
				else
...
txstart(Ether* ether)
{
	...
	if(ctlr->ram)
		memmove((void*)(ether->_isaconf.mem+ctlr->tstart*Dp8390BufSz), rp, len);
	else
...
interrupt(Ureg* ureg, void* arg)
{
	...
	while(isr = (regr(ctlr, Isr) & (Cnt|Ovw|Txe|Rxe|Ptx|Prx))){
		...
		if(isr & Cnt){
			ether->_netif.frames += regr(ctlr, Ref0);
			ether->_netif.crcs += regr(ctlr, Ref1);
			ether->_netif.buffs += regr(ctlr, Ref2);
			regw(ctlr, Isr, Cnt);
		}
	}
...
setfilter(Ether *ether, Dp8390 *ctlr)
{
	...
	r = Ab;
	mar = 0;
	if(ether->_netif.prom){
		r |= Pro|Am;
		mar = allmar;
	} else if(ether->_netif.nmaddr){
		r |= Am;
		mar = ctlr->mar;
	}
	if(mar){
