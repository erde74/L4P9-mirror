...
int
ioreserve(int _x, int size, int align, char *tag)
{
	IOMap *m, **l;
	int i, port;

	lock(&iomap._lock);  //% (&iomap)
...
static int  //% void
checkport(int start, int end) //% ONERR
{
...
static long
archread(Chan *c, void *a, long n, vlong offset)  //% ONERR
{
	char *buf, *p;
	int port;
	ushort *sp;
	...
	case Qiob:
		port = offset;
		rc = checkport(offset, offset+n);  //% ONERR
...
static long
archwrite(Chan *c, void *a, long n, vlong offset)  //% ONERR
{
	char *p;
	int port;
	ushort *sp;
...
static long
cputyperead(Chan* _x, void *a, long n, vlong offset)
{
	char str[32];
	ulong mhz;
...
static long
archctlread(Chan* _x, void *a, long nn, vlong offset)
{
	char buf[256];
	int n;
...
static long
archctlwrite(Chan* _x, void *a, long n, vlong _y)  //% ONERR
{
	Cmdbuf *cb;
	Cmdtab *ct;
	...
	case CMpge:
		if(!m->havepge)
		  ERROR_GOTO("processor does not support pge", _ERR_1);  //%
	...
	case CMcoherence:
		if(strcmp(cb->f[1], "mb386") == 0)
			...
			if(X86FAMILY(m->cpuidax) < 5)
			        ERROR_RETURN("invalid coherence ctl on this cpu family", ONERR);   //%
			coherence = mb586;
		}
		else if(strcmp(cb->f[1], "nop") == 0){
			/* only safe on vmware */
			if(conf.nmach > 1)
			        ERROR_RETURN("cannot disable coherence on a multiprocessor", ONERR);  //%
			coherence = nop;
		}else
