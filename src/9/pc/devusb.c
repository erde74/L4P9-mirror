...
#define DPRINT if(Chatty)l4printf_b
#define XPRINT if(debug)l4printf_b
#else //plan9 --------------------------------------------
...
static Udev*
usbnewdevice(Usbhost *uh)  //% NIL on err
{
	int i;
	Udev *d;
	Endpt *e;
...
static int
usbgen(Chan *c, char * _x, Dirtab* _y, int _z, int s, Dir *dp)   //%
{
	Qid q;
	Udev *d;
	Endpt *e;
...
usbprobe(int cardno, int ctlrno)
{
	...
	if(cardno < 0){  DBGPRN("usbprobe:1 ");
	        if(isaconfig("usb", ctlrno, &uh->_isaconf) == 0){   //%  uh
			free(uh);
			return nil;
		}  
		DBGPRN("usbprobe:2 ");
		...
		for(cardno = 0; usbtypes[cardno].type; cardno++){
		        type = uh->_isaconf.type;  //% _isaconf.
			if(type==nil || *type==0)
				type = "uhci";
	 ...
	 */
	if(uh->_isaconf.irq == 2)        //% _isaconf
	        uh->_isaconf.irq = 9;    //% _isaconf

	snprint(name, sizeof(name), "usb%d", ctlrno);
	intrenable(uh->_isaconf.irq, uh->interrupt, uh, uh->tbdf, name);  //% _isaconf

	ebuf = buf + sizeof buf;
	p = seprint(buf, ebuf, "#U/usb%d: %s: port 0x%luX irq %d", 
		    ctlrno, usbtypes[cardno].type, 
		    uh->_isaconf.port, uh->_isaconf.irq);     //% _isaconf x 2

	if(uh->_isaconf.mem)    //% _isaconf
	        p = seprint(p, ebuf, " addr 0x%luX", 
			    PADDR(uh->_isaconf.mem));   //% _isaconf

	//% l4printf_g("uh->_isaconf.mem=<%x %x>\n", 
	//%	   uh->_isaconf.mem, PADDR(uh->_isaconf.mem));

	if(uh->_isaconf.size)    //% _isaconf
	        seprint(p, ebuf, " size 0x%luX", uh->_isaconf.size);   //% _isacoonf
	print("%s\n", buf);
...
Chan*
usbopen(Chan *c, int omode)  //% NIL on err
{
	Udev *d;
	Endpt *e;
	int f, s, type;
	...
	if(type == Qnew){
		...
		if(d == nil) {
			...
			ERROR_RETURN(Enodev, nil);    //% error(Enodev);
		}
		type = Qctl;
	...
	case Qctl:
		if(0&&d->busy)
		        ERROR_GOTO(Einuse, _ERR_1);  //% error(Einuse);
		d->busy = 1;
		if(!f)
		        incref(&d->_ref);  //% (d)
...
long
usbread(Chan *c, void *a, long n, vlong offset)  // ONERR on error
{
	int t, i;
	Udev *d;
	Endpt *e;
...
long
usbwrite(Chan *c, void *a, long n, vlong offset)  //% ONERR on error
{
	Udev *d;
	Endpt *e;
	Cmdtab *ct;
	...
	case Qctl:
		...
		case CMdata:
			...
			if(i < 0 || i >= nelem(d->ep) || d->ep[i] == nil)
			        ERROR_GOTO(Ebadusbmsg, _ERR_2);  //% error(Ebadusbmsg);
		...
		case CMadjust:
			...
			if (i < (e->maxpkt-1)/e->samplesz * 1000/e->pollms
				...
				ERROR_GOTO(cmd, _ERR_2);  //% error(cmd);
			}
			e->hz = i;
			break;
		...
		case CMep:
			...
			if(i < 0 || i >= nelem(d->ep)) {
				...
				ERROR_GOTO(Ebadarg, _ERR_2);  //% error(Ebadarg);
			}
			...
			_ERR_3:
				freept(e);
				qunlock(&uh->_qlock);  //% (uh)
				NEXTERROR_GOTO(_ERR_2);  //% nexterror();
			}

			if(e->active)
			        ERROR_GOTO(Eperm, _ERR_3);  //% error(Eperm);
			...
			if(strcmp(cb->f[2], "bulk") == 0){
				...
				if(i >= 1 && i <= 8){
					...
					ERROR_GOTO(Ebadarg, _ERR_3);  //% error(Ebadarg);
				}
