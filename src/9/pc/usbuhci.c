...
struct Ctlr
{
        Lock    _lock;	/* protects state shared with interrupt (eg, free list) */ //%
	Ctlr*	next;
	Pcidev*	pcidev;
	int	active;

	int	io;
	ulong*	frames;	/* frame list.  ctlr->frames[i]; Phys-addr */
...
alloctd(Ctlr *ctlr)
{
	...
	ctlr->freetd = t->next;
	t->next = nil;
	iunlock(&ctlr->_lock );   //% iunlock(ctlr)
	t->ep = nil;
	t->bp = nil;
	t->status = 0;
	t->link = Terminate;
...
eptactivate(Ctlr *ctlr, Endpt *e)
{
	...
	ilock(&ctlr->activends._lock);  //% (&ctlr->activends);
	if(e->active == 0){
	...
	iunlock(&ctlr->activends._lock);  //% (&ctlr->activends);
}
...
static int  //% void
epopen(Usbhost *uh, Endpt *e)  //% ONERR on error
{
	Ctlr *ctlr;
	DBGPRN(">%s()\n", __FUNCTION__);
	...
	ctlr = uh->ctlr;
	if(e->iso && e->active)
	        ERROR_RETURN("already open", ONERR); //% error("already open");

	if(schedendpt(ctlr, e) < 0){
		if(e->active)
		        ERROR_RETURN("cannot schedule USB endpoint, active", ONERR);
		        //% error("cannot schedule USB endpoint, active");
		else
		        ERROR_RETURN("cannot schedule USB endpoint", ONERR);
...
static int  //% void
portreset(Usbhost *uh, int port)  //% ONERR on error
{
	int i, p;
	Ctlr *ctlr;
	DBGPRN(">%s()\n", __FUNCTION__);
...
static int  //% void
portenable(Usbhost *uh, int port, int on)  //% ONERR on error
{
	int w, p;
	Ctlr *ctlr;
	DBGPRN(">%s()\n", __FUNCTION__);
...
interrupt(Ureg* _x, void *a)
{
	...
	int s, frnum;
	Usbhost *uh;
	DBGPRN(">%s(irq=%d)\n", __FUNCTION__, ((Usbhost*)a)->_isaconf.irq );
...
static long
isoio(Ctlr *ctlr, Endpt *e, void *a, long n, ulong offset, int w)  //% ONERR on error
{
	TD *td;
	Endptx *x;
	int i, frnum;
	...
	if (offset != 0 && offset != e->foffset){
		...
		if (offset < td->offset)
		        ERROR_GOTO("ancient history", _ERR_1);  //% error("ancient history");
...
static long
read(Usbhost *uh, Endpt *e, void *a, long n, vlong offset)  //% ONERR
{
	long l, i;
	Block *b;
	Ctlr *ctlr;
	...
	do {
		...
		if(e->err)
		        ERROR_GOTO(e->err, _ERR_1);  //% error(e->err);
...
static long
write(Usbhost *uh, Endpt *e, void *a, long n, vlong offset, int tok)  //% ONERR on error
{
	int i, j;
	QH *qh;
	Block *b;
...
reset(Usbhost *uh)
{
	...
	for(ctlr = ctlrhead; ctlr != nil; ctlr = ctlr->next){
		if(ctlr->active)
			continue;
		if(uh->_isaconf.port == 0 || uh->_isaconf.port == ctlr->io){  //% _isaconf _
			ctlr->active = 1;
			break;
		}
	}
	...
	uh->ctlr = ctlr;
	uh->_isaconf.port = io;       //% _isaconf
	uh->_isaconf.irq = p->intl;   //% _isaconf
	uh->tbdf = p->tbdf;
	...
	DBGPRN("<%s()io:0x%x irq=%d\n", 
	       __FUNCTION__, uh->_isaconf.port, uh->_isaconf.irq);
