...
#define  _DBGDO   0    // 0  or 1
static int  _DBGFLG = _DBGDO;   // In this file, _DBGFLG must be a variable.
#include  <l4/_dbgpr.h>

#define  DBGON   (_DBGFLG = _DBGDO)
#define  DBGOFF  (_DBGFLG = 0)
...
struct Ctlr {
	...
	Prd*	prdt;			/* physical region descriptor table */
        ...
        QLock   _qlock;	   //%	 /* current command */
	Drive*	curdrive;
	int	command;		/* last command issued (debugging) */
        Rendez  _rendez;   //%
	int	done;

        Lock    _lock;	  //%	/* register access */
} ;
...
atadumpstate(Drive* drive, uchar* cmd, vlong lba, int count)
{
	...
	ctlr = drive->ctlr;
	print("atadumpstate command %2.2uX\n", ctlr->command);
...
ataready(int cmdport, int ctlport, int dev, int reset, int ready, int micro)
{
	...
	atadebug(cmdport, ctlport, "ataready: dev %uX reset %uX ready %uX",
		dev, reset, ready);
	DBGPRN(">ataready(%x %x %x %x %x %x) \n", 
	    cmdport, ctlport, dev, reset,  ready, micro);
	...
	for(;;){
		 ...
		 */
	        as = inb(ctlport+As);   //DBGBRK("ataready 3\n");
		if(as & reset){
			/* nothing to do */
		}
		else if(dev){  
		...
		DBGPRN("* inb loop *\n");  DBGOFF;  //%
		if(micro-- <= 0){   //%%%%%%%% longtime wait
			atadebug(0, 0, "ataready: %d 0x%2.2uX\n", micro, as);
			break;
		} 
		microdelay(1); 
	}
	DBGON;  DBGPRN("<* inb loop *\n");//%
...
{
	int as, command, drdy;
	DBGPRN("> ataidentify(%x, %x, %x, %x, %x)\n", cmdport, ctlport, dev, pkt,  info);
	if(pkt){
		command = Cidpkt;
		drdy = 0;
	}
	...
	//%%%%%%%% Here, it may wait 103 milli-secs 
	if(as < 0)
		return as;
...
atadrive(int cmdport, int ctlport, int dev)
{
	...
	uchar buf[512], *p;
	ushort iconfig, *sp;
	DBGPRN("> atadrive(%x, %x, %x) \n", cmdport, ctlport, dev);
	atadebug(0, 0, "identify: port 0x%uX dev 0x%2.2uX\n", cmdport, dev);
	pkt = 1;
retry:
	as = ataidentify(cmdport, ctlport, dev, pkt, buf);
	if(as < 0) {   DBGPRN("< atadrive nil \n");
		return nil;
	}
	if(as & Err){
	        if(pkt == 0) {  DBGPRN("< atadrive nil \n");
			return nil;
		}
		pkt = 0;
		goto retry;
	...
	if((drive = malloc(sizeof(Drive))) == nil) { DBGPRN("< atadrive nil \n");
		return nil;
	}
	drive->dev = dev;
...
ataprobe(int cmdport, int ctlport, int irq)
{
	...
	PRN(">ataprobe(%x, %x, %x) ", cmdport, ctlport, irq); //%%
...
tryedd1:
	...
	sdev->nunit = 2;
	ctlr->sdev = sdev;
	PRN("<ataprobe<idno=%c %s> \n", sdev->idno, (dev==Dev0)?"Master":"Slave");
	//L4_KDB_Enter("ataprobe");
	return sdev;
}
...
static SDev*
ataprobew(DevConf *cf)  //% ONERRNIL
{
	char *p;
	ISAConf isa;
	DBGPRN("> %s(%x) \n", __FUNCTION__, cf);	
	if (cf->nports != 2)
	        ERROR_RETURN(Ebadarg, nil);  //% error(Ebadarg);

	memset(&isa, 0, sizeof isa);
	isa.port = cf->ports[0].port;
	isa.irq = cf->intnum;
	if((p=strchr(cf->type, '/')) == nil || pcmspecial(p+1, &isa) < 0)
	        ERROR_RETURN("cannot find controller", nil); //% error("cannot find controller");
...
atastandby(Drive* drive, int period)
{
	...
	iunlock(& ctlr->_lock );  //%   <- (ctlr)

	while(WASERROR()){  //%
	_ERR_1:
		;
	}
	tsleep( &ctlr->_rendez, atadone, ctlr, 60*1000);  //% ctlr
	POPERROR();  //%
...
atapktio(Drive* drive, uchar* cmd, int clen)
{
...
#if 0 //%-----------------------
	l4printf_r("atapktio(%x %x) is skipped !!\n", cmd[0], cmd[1]);
	return SDok;
#endif //-----------------------
	DBGPRN(">%s(%x %x) ", __FUNCTION__, cmd[0], cmd[1]);
	...
	DBGPRN("%s:sleep  ", __FUNCTION__); //%
	while(WASERROR()){  //%
	_ERR_1:
		;
	}
	if(!drive->pktdma)
	        sleep(& ctlr->_rendez, atadone, ctlr);  //%
...
static int
atagenio(Drive* drive, uchar* cmd, int _x)
{
	uchar *p;
	Ctlr *ctlr;
	vlong lba, len;
	...
	while(count){
		...
		tsleep(&ctlr->_rendez, atadone, ctlr, 60*1000); //% (ctrl,,)
	...
	if(status == SDretry){
		if(DbgDEBUG)
			print("%s: retry: dma %8.8uX rwm %4.4uX\n",
			      unit->_sdperm.name, drive->dmactl, drive->rwmctl); //% _sdperm
		goto retry;
	}
	if(status == SDok){
...
static void
atainterrupt(Ureg* _u, void* arg)
{
	Ctlr *ctlr;
	Drive *drive;
	int cmdport, len, status;
	DBGPRN("> ATAINTERRUPT(%x, %x) \n", _u, arg);
	check_clock('S', "intr");  //%%
...
atarctl(SDunit* unit, char* p, int l)
{
	...
	n += snprint(p+n, l-n, "\n");
	if(drive->sectors){
	  n += snprint(p+n, l-n, "geometry %ld %d", //% %lld
...
static int
atawctl(SDunit* unit, Cmdbuf* cb)  //% ONERR
{
	int period;
	Ctlr *ctlr;
	Drive *drive;
	DBGPRN("> %s(%x,%x) \n", __FUNCTION__, unit, cb);
