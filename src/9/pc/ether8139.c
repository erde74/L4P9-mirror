 ...
 * Date    : 2008/05/25
 *
 * Realtek 8139 ethernet driver in LP49.
 */
...
//% <K.Hayashi>
//% Modify the definition of "struct Ctlr".
...
check_clock('g',  "8139recv");// 
	...
	while(!(csr8r(ctlr, Cr) & Bufe)){
		...
		if(!(status & Rcok)){
			if(status & (Ise|Fae))
				edev->_netif.frames++; //% <K.Hayashi> _netif
			if(status & Crc)
				edev->_netif.crcs++; //% <K.Hayashi> _netif
			if(status & (Runt|Long))
				edev->_netif.buffs++; //% <K.Hayashi> _netif
...
//% <K.Hayashi>
//% The first argument name is added.
static void
...
check_clock('g',  "8139intr");// 
	...
	while((isr = csr16r(ctlr, Isr)) != 0){
		...
		if(isr & (Ter|Tok)){
			...
			while(ctlr->ntd){
				...
				if(!(tsd & Tcok)){
					...
					edev->_netif.oerrs++; //% <K.Hayashi> _netif
				}
		...
		if(isr & PunLc){
			...
			msr = csr8r(ctlr, Msr);
			if(!(msr & Linkb)){
				if(!(msr & Speed10) && edev->_netif.mbps != 100){ //% <K.Hayashi> _netif
					edev->_netif.mbps = 100; //% <K.Hayashi> _netif
					qsetlimit(edev->oq, 256*1024);
				}
				else if((msr & Speed10) && edev->_netif.mbps != 10){ //% <K.Hayashi> _netif
					edev->_netif.mbps = 10; //% <K.Hayashi> _netif
...
rtl8139match(Ether* edev, int id)
{
	...
	for(ctlr = ctlrhead; ctlr != nil; ctlr = ctlr->next){
		...
		if(edev->_isaconf.port != 0 && edev->_isaconf.port != port) //% <K.Hayashi> _isaconf
			continue;
...
rtl8139pnp(Ether* edev)
{
    ...
    //% <K.Hayashi>
    //% Check message.
    //% print("<K.Hayashi> : \"rtl8139pnp\" is run by \"etherprobe\".\n");
	 ...
	 */
	id = 0;
	for(i = 0; i < edev->_isaconf.nopt; i++){ //% <K.Hayashi> _isaconf
		if(cistrncmp(edev->_isaconf.opt[i], "id=", 3) == 0) //% <K.Hayashi> _isaconf
			id = strtol(&edev->_isaconf.opt[i][3], nil, 0); //% <K.Hayashi> _isaconf
	}
	//% print("<K.Hayashi> : id=%d\n", id); //% XXX id=0
	ctlr = nil;
	if(id != 0)
    ...
    */
    if(ctlr == nil) {
      //% print("<K.Hayashi> : \"rtl8139pnp\" returns -1.\n");
        return -1;
    }

	edev->ctlr = ctlr;
	edev->_isaconf.port = ctlr->port; //% <K.Hayashi> _isaconf
	edev->_isaconf.irq = ctlr->pcidev->intl; //% <K.Hayashi> _isaconf
	...
	edev->_netif.arg = edev; //% <K.Hayashi> _netif
	edev->_netif.promiscuous = rtl8139promiscuous; //% <K.Hayashi> _netif
    ...
    //% <K.Hayashi>
    //% Check message.
    //% print("<K.Hayashi> : \"rtl8139pnp\" returns 0.\n");
