...
ifstat(Ether* ether, void* a, long n, ulong offset)
{
	...
	ether->_netif.crcs = ctlr->crc;   //% _netif
	ether->_netif.frames = ctlr->fram;   //% _netif
	ether->_netif.buffs = ctlr->rxbuff+ctlr->txbuff;  //% _netif
	ether->_netif.overflows = ctlr->oflo;   //% _netif
...
//	DBGPRN("** interrupt **\n");
	 ...
	 */
	if(csr0 & Rint){
	        //  PRN("#eth-interrupt:Receiver\n");
		check_clock('l', "recv-intr");
...
//			DBGPRN(" iallocb rc=0x%x\n",bp);
	 ...
	 */
	if(csr0 & Tint){
//		DBGPRN("   interrupt : Transmitter\n");
		check_clock('l', "xmit-intr");

	        lock(&ctlr->_lock);      //% lock(ctlr);
		while(ctlr->ntq){
...
amd79c970pci(void)
{
  ...
  DBGBRK("ctlrhead=0x%08x, chlrtail->next=0x%08x\n", ctlrhead, ctlrtail->next);
  DBGPRN("<<< %s:%s (return void)\n", __FILE__, __FUNCTION__);
}
...
reset(Ether* ether)
{
  DBGPRN(">>> %s:%s\n", __FILE__, __FUNCTION__);
	...
	for(ctlr = ctlrhead; ctlr != nil; ctlr = ctlr->next){
		if(ctlr->active)
			continue;
		if(ether->_isaconf.port == 0 || ether->_isaconf.port == ctlr->port){ //% _isaconf.
			ctlr->active = 1;
			break;
		}
	}
	if(ctlr == nil){
  DBGPRN("ether->_isaconf.port=%d\n", ether->_isaconf.port);
  DBGPRN("<<< %s:%s (return -1)1\n", __FILE__, __FUNCTION__);
		return -1;
	}
	 ...
	 */
	ether->ctlr = ctlr;
	ether->_isaconf.port = ctlr->port;  //% _isaconf
	ether->_isaconf.irq = ctlr->pcidev->intl;   //% _isaconf
  ...
  DBGPRN("<<< %s:%s (return -1)3\n", __FILE__, __FUNCTION__);
	...
	ether->_netif.arg = ether;   //% _netif.
	ether->_netif.promiscuous = promiscuous;   //%  _netif.
