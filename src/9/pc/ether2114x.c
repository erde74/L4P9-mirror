...
static void
multicast(void* a_on, uchar* b_on, int c_on)
{
}
...
ifstat(Ether* ether, void* a, long n, ulong offset)
{
	...
	ether->_netif.crcs = ctlr->ce;
	ether->_netif.frames = ctlr->rf+ctlr->cs;
	ether->_netif.buffs = ctlr->de+ctlr->tl;
	ether->_netif.overflows = ctlr->of;
...
static int
typephylink(Ctlr* ctlr, uchar* on)
{
...
static int
type2mode(Ctlr* ctlr, uchar* block, int on)
{
	uchar *p;
...
reset(Ether* ether)
{
	...
	for(i = 0; i < ether->_isaconf.nopt; i++){
		if(cistrcmp(ether->_isaconf.opt[i], "FD") == 0){
			ctlr->fd = 1;
			continue;
		}
		for(x = 0; x < nelem(mediatable); x++){
			debug("compare <%s> <%s>\n", mediatable[x],
				ether->_isaconf.opt[i]);
			if(cistrcmp(mediatable[x], ether->_isaconf.opt[i]))
				continue;
			ctlr->medium = x;
	...
	ether->_netif.mbps = media(ether, 1);
