...
static void nullfunc(char * fmt, ...) { }
 
#if 1
int l4printf_b(char*, ...);
char l4printgetc();
  #define  DBGPRN  if(DBGFLG) l4printf_b
  #define  DBGBRK  l4printgetc
#else
  #define  DBGPRN  if(0) nullfunc
  #define  DBGBRK  if(0) nullfunc
#endif
...
void * malloc(size_t size)
{
  ...
  for (ntries = 0; ntries < 2; ntries++) {
	...
	for (prev = 0, p = _empty; p != 0; prev = p, p = NextFree(p)) {
		...
		{ // devsd.c and others (?) require zero clear !!
		  int i; 
...
static void* memset(void *ap, int c, unsigned n)
{
  char *p;
   p = ap;
  while(n > 0) {
...
void * mallocz(unsigned size, int clr)
{
  void *v;
  v = malloc(size);
  if (clr && v != 0)
...
void setmalloctag(void *v, unsigned pc)
{
  DBGPRN("! %s \n", __FUNCTION__);
}
...
int main( )
{
  ...
  for (i = 1; i<1000; i++)
    {
      p = malloc(i * 8);
      if (p == (char*)0)  l4printf("XXXXXX\n");

      sum += i*8;
      for(q = p, j=0; j<i*8; q++, j++) 
	*q = '#'; 

      *q = 0;
      l4printf("[%x %x] %s\n", p, sum, p);
      if (i%10 == 0) L4_KDB_Enter("");
    }
  return 0;
}
#endif 
