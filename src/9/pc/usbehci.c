 ...
 */
struct Qio {
        QLock   _qlock;	//%	/* for the entire I/O process */
        Rendez  _rendez;//%	/* wait for completion */
...
struct Ctlio {
        Qio  _qio; //%		/* a single Qio for each RPC */
...
struct Isoio {
        QLock  _qlock; //%
        Rendez _rendez;//%	/* wait for space/completion/errors */
...
struct Ctlr {
        Rendez  _rendez;//%	/* for waiting to async advance doorbell */
        Lock   _lock;//%	/* for ilock. qh lists and basic ctlr I/O */
...
static int
Vportenable(Hci *hp, int port, int on)
{
	Ctlr *ctlr;
	Eopio *opio;
	int s;
