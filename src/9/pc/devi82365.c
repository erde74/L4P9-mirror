...
increfp(PCMslot *pp)
{
	lock(&pp->_lock);  //%  lock(pp);
	if(pp->ref++ == 0)
		slotena(pp);
	unlock(&pp->_lock);  //%  lock(pp);
}
...
static int
pcmgen(Chan *c, char* _x, Dirtab * _y, int _z, int i, Dir *dp)  //%
{
	int slotno;
	Qid qid;
	long len;
...
static Chan*
i82365open(Chan *c, int omode)  //% ONERR nil
{
	if(c->qid.type & QTDIR){
		if(omode != OREAD)
...
static long
pcmread(int slotno, int attr, void *a, long n, vlong off) //% ONERR
{
	int i, len;
	PCMmap *m;
	uchar *ac;
	...
	for(len = n; len > 0; len -= i){
		...
		if(m == 0)
		        ERROR_GOTO("cannot map PCMCIA card", _ERR_1);  //%
		if(offset + len > m->cea)
			i = m->cea - offset;
		else
			i = len;
...
static long
i82365read(Chan *c, void *a, long n, vlong off)  //% ONERR
{
	char *p, *buf, *e;
	PCMslot *pp;
	ulong offset = off;
	...
	case Qmem:
	case Qattr:
	        return pcmread(SLOTNO(c), TYPE(c) == Qattr, a, n, off); //% ONERR
	case Qctl:
...
static long
pcmwrite(int dev, int attr, void *a, long n, vlong off)  //% ONERR
{
	int i, len;
	PCMmap *m;
	uchar *ac;
...
static long
i82365write(Chan *c, void *a, long n, vlong off)  //% ONERR
{
	PCMslot *pp;
	char buf[32];
