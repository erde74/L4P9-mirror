...
control(Biobuf *bp, int c)
{
	...
	switch(c){
	case AreYouThere:
	  fprint(Bfildes(&bp->_Biobufhdr), "Plan 9 telnet, version 1\r\n"); //% bp
		break;
	case Sb:
		return sub(bp);
	case Will:
...
will(Biobuf *bp)
{
	...
	c = Bgetc(&bp->_Biobufhdr);   //% bp
	if(c < 0)
		return -1;
	DPRINT(2, "will %d\n", c);
	o = findopt(c);
	if(o == 0){
	  send3(Bfildes(&bp->_Biobufhdr), Iac, Dont, c);  //% bp
		return 0;
	}
	if(o->noway)
	  send3(Bfildes(&bp->_Biobufhdr), Iac, Dont, c);  //% bp
	else if(o->remote == 0)
		rv |= send3(Bfildes(&bp->_Biobufhdr), Iac, Do, c);
	if(o->remote == 0){
		if(o->change)
			rv |= (*o->change)(bp, Will);
	}
...
wont(Biobuf *bp)
{
	...
	if(o->remote){
		if(o->change)
			rv |= (*o->change)(bp, Wont);
		rv |= send3(Bfildes(&bp->_Biobufhdr), Iac, Dont, c);  //% bp
	}
	o->remote = 0;
	return rv;
}
...
doit(Biobuf *bp)
{
	...
	o = findopt(c);
	if(o == 0 || o->noway){
	  send3(Bfildes(&bp->_Biobufhdr), Iac, Wont, c);  //% bp
		return 0;
	}
	if(o->noway)
		return 0;
	if(o->local == 0){
		if(o->change)
			rv |= (*o->change)(bp, Do);
		rv |= send3(Bfildes(&bp->_Biobufhdr), Iac, Will, c);  //% bp
	}
	o->local = 1;
	return rv;
}
...
dont(Biobuf *bp)
{
	...
	if(o->local){
		...
		if(o->change)
			rv |= (*o->change)(bp, Dont);
		rv |= send3(Bfildes(&bp->_Biobufhdr), Iac, Wont, c);   //% bp
	}
	o->local = 0;
	return rv;
}
