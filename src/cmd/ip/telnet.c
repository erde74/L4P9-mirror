...
usage(void)
{
	fatal("usage: telnet [-Cdnr] ipaddr [port]", 0, 0);
...
#if TRIAL //% trial ------------------
int
dodial2(char *hostip, char*port)
{
	char name[32];
	int data;
	char devdir[NETPATHLEN];

	snprint(name, sizeof(name), "tcp!%s!%s", hostip, port);
	PRN(">dodial2(\'%s\' \'%s\'): \'%s\'\n", hostip, port, name);
		...
		if(!opt[Binary].local){
			if(c == '\n'){
				...
				if(opt[SGA].remote){
					...
					if(Bputc(&ob._Biobufhdr, '\r') < 0)   //% &ob 
						return;
				}
			}
		}
		if(Bputc(&ob._Biobufhdr, c) < 0)   //% &ob 
			return;
		if(Bbuffered(&ib._Biobufhdr) == 0)   //% &ib 
			if(Bflush(&ob._Biobufhdr) < 0)   //% &ob 
				return;
	}
}
...
menu(Biobuf *bp, int net)
{
	...
	fprint(2, ">>> ");
	for(done = 0; !done; ){
		cp = Brdline(&bp->_Biobufhdr, '\n');      //% bp 
		if(cp == 0){
			comm->stopped = 0;
			return -1;
		}
		cp[Blinelen(&bp->_Biobufhdr)-1] = 0;      //% bp 
		switch(*cp){
		case '!':
			system(Bfildes(&bp->_Biobufhdr), cp+1);     //% bp 
			done = 1;
			break;
		case '.':
			done = 1;
...
termsub(Biobuf *bp, uchar *sub, int n)
{
	...
	if(sub[0] == 1){
		...
		*p++ = Iac;
		*p++ = Se;
		return iwrite(Bfildes(&bp->_Biobufhdr), buf, p-buf);  //% bp
	}
	return 0;
}
