...
extern L4_ThreadId_t wait_activation(int ms);
extern int post_activation(L4_ThreadId_t tid);
...
#if INCORE //%--- started in boot phase -------------
void
main(int argc, char **argv)
{
...

	{
	  ...
	  //%		open("/dev/null", OWRITE);
		...
		close(pipefd[0]);
		fprint(2, "9660srv %d: serving %s\n", getpid(), srvfile);
	}
	srvfd = pipefd[1];

	ack2hvm(0); //090330

	io(srvfd);  // Loop for ever 
	exits(0);
}

#else //%--- started up by user ----------------
void
main(int argc, char **argv)
{
	int srvfd, pipefd[2], stdio;
