...
extern void dbg_memory(char *title, unsigned char *start, unsigned size);

#define   DXX   if(0)l4printf_b
...
main(int argc, char **argv)
{
	...
	case 'S':   // [ -S service ] 
		if(srvfdfile)
			usage();
		srvfdfile = EARGF(usage());    //% srvfile = 'service'
		break;
...
//		Workq[i].buf = emallocz(messagesize);
	...
	else {  // initial protocol to get tree-root
		buf[0] = 0;
		n = read(0, buf, sizeof(buf)-1);  // <-> import
		if(n < 0) {
	...
	if (readn(netfd, &initial, sizeof(ulong)) < sizeof(ulong))  //% Ex. "impo nofilter clear"
	  {  // _d("up",0);L4_KDB_Enter("EX75"); 
	     fatal("can't read initial string: %r\n");}
{//DBG
  char pathname[64];
  int  rc;
  rc=fd2path(netfd, pathname, 64);
  DXX("\n#92 netfd:%d rc:%d path:'%s' initial:'%c%c%c%c'\n", 
      netfd, rc, pathname, initial&0xFF, initial>>8&0xFF, initial>>16&0xFF, initial>>24&0xFF);
 }
...
DXX("Err:exportfs:reply<netfd=%d m=%d n=%d>\n", netfd, m, n);
...
fatal(char *s, ...)
{
	...
	//%	        postnote(PNPROC, m->pid, "kill");  //% must be xared !

	print("fatal:exportfs:%s\n", buf); //%
