...
/* already in plan9.h #include <sys/types.h> */
/* for struct passwd, struct group, struct stat ... */
...
DBGPRN(2,">oldfidex-10() ");  //%

	if (auth != -1 && f->auth != auth) {
		*ep = Ebadfid;
		return nil;
	}
DBGPRN(2,">oldfidex-20() ");  //%
