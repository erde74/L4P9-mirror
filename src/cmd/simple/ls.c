...
format(Dir *db, char *name)
{
	...
	if(sflag)
	  Bprint(&bin._Biobufhdr, "%*llud ",   //%
			swidth, (db->length+1023)/1024);
	if(mflag){
	  Bprint(&bin._Biobufhdr, "[%s] ", db->muid);    //%
		for(i=2+strlen(db->muid); i<mwidth; i++)
			Bprint(&bin._Biobufhdr, " ");
	}
	if(qflag)
	  Bprint(&bin._Biobufhdr, "(%.16llux %*lud %.2ux) ",  //%
			db->qid.path,
			qwidth, db->qid.vers,
			db->qid.type);
	if(Tflag)
	  Bprint(&bin._Biobufhdr, "%c ", (db->mode&DMTMP) ? 't' : '-'); //%
