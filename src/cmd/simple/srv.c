...
static int _amount(int fd, char *mntpt, int flags, char *aname)
{
  int rv, afd = -1;
  //%  AuthInfo *ai;
...
main(int argc, char *argv[])
{
	...
	print("srv{domount=%d rmount=%d doexec=%d srv='%s' mtpt='%s' \n", 
	      domount, reallymount, doexec, srv, mtpt);
...
Mount:
	...
	if(_amount(fd, mtpt, mountflag, "") < 0){
		err[0] = 0;
