...
pcihinv(Pcidev* p)
{
	...
	if(p == nil) {
		p = pciroot;
		Bprint(&stdout._Biobufhdr, "bus dev type vid  did intl memory\n");	// HK 20090831
	}
	for(t = p; t != nil; t = t->link) {
		Bprint(&stdout._Biobufhdr, "%d  %2d/%d %.4ux %.4ux %.4ux %2d  ",	// HK 20090831
		...
		for(i = 0; i < nelem(p->mem); i++) {
			if(t->mem[i].size == 0)
				continue;
			Bprint(&stdout._Biobufhdr, "%d:%.8lux %d ", i,	// HK 20090831
