...
vgactlinit(void)
{
	...
	for(nl = strchr(ctlbuf, '\n'); nl; nl = strchr(nl, '\n')){
		...
		*nl = '\0';
		if((p = strchr(vp, ' ')) != (void *) 0){		// HK 20090831
			*p++ = '\0';
			attr[nattr].attr = vp;
			if(*p == '\0')
...
dumpbios(long size)
{
	...
	for(i = 0; i < size; i += 16){
		Bprint(&stdout._Biobufhdr, "0x%luX", offset+i);	// HK 20098031
		for(n = 0; n < 16; n++)
			Bprint(&stdout._Biobufhdr, " %2.2uX", buf[i+n]);	// HK 20090831
		Bprint(&stdout._Biobufhdr, "  ");		// HK 20090831
		for(n = 0; n < 16; n++){
			c = buf[i+n];
			if(c < 0x20 || c >= 0x7F)
				c = '.';
			Bprint(&stdout._Biobufhdr, "%c", c);	// HK 20090831
		}
...
printitem(char* ctlr, char* item)
{
	...
	n = 0;
	if(ctlr && *ctlr)
		n = Bprint(&stdout._Biobufhdr, "%s ", ctlr);	// HK 20090831
	Bprint(&stdout._Biobufhdr, "%-*s", 20-n, item);		// HK 20090831
}
...
printreg(ulong data)
{
	...
	if(curprintindex == 8)
		Bprint(&stdout._Biobufhdr, " -");		// HK 20090831
	Bprint(&stdout._Biobufhdr, "%*.2luX", width, data);	// 20090831
	curprintindex++;
}
...
printflag(ulong flag)
{
	...
	for(i = 31; i >= 0; i--){
		if((flag & (1<<i)) == 0)
			continue;
		if(flagname[i])
			Bprint(&stdout._Biobufhdr, "%c%s", first, flagname[i]);		// HK 20090831
		else
			Bprint(&stdout._Biobufhdr, "%c0x%x", first, 1<<i);		// HK 20090831
		first = '|';
	}
}
