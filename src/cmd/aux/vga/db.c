...
dbopen(char* dbname)
{
	...
	if((db = ndbopen(dbname)) == (void *) 0) {	// HK 20090831
		//error("dbopen: %s: %r\n", dbname);	// HK 20090831
		return 0;	// HK 20090831
	}
	return db;
}
...
addctlr(Vga* vga, char* val)
{
	...
	name[Namelen] = 0;
	if ((p = strchr(name, '-')) != nil)	// HK 20090930
		*p = 0;
...
dbbios(Vga *vga, Ndbtuple *tuple)
{
	...
	for(t = tuple->entry; t; t = t->entry){
		...
		if ((p = strchr(t->attr, '-')) != nil) {	// HK 20090930
	...
	for(t = tuple->entry; t; t = t->entry){
		...
		for(td = t->line; td != t; td = td->line){
			...
			if(strcmp(td->val, "*") == 0)
				...
				continue;
			for(pci = nil; (pci = pcimatch(pci, vid, did)) != nil;)	// HK 20090930
				if((pci->ccru>>8) == 3)
					break;
			if(pci == nil)
				continue;
...
//	Ctlr *c;	// HK 20090930
	...
	for(t = tuple->entry; t; t = t->entry){
		...
		//	if(strcmp(t->val, "vesa") == 0)	// HK 20090930
		//		vga->vesa = vga->ctlr;	// HK 20090930
			...
			/* c = */ addctlr(vga, t->val);
		//	if(strcmp(t->val, "vesa") == 0)	// HK 2009030
		//		vga->vesa = c;		// HK 2009030
...
dbmonitor(Ndb* db, Mode* mode, char* type, char* size)
{
	...
	clock = 0;
	strcpy(buf, size);
	if ((p = strchr(buf, '@')) != nil) {	// HK 20090930
		*p++ = 0;
	...
	if ((p = ndbgetvalue(db, &s, attr, "", "videobw", nil)) != nil) {	// HK 20090930
...
dbdumpmode(Mode* mode)
{
	...
	Bprint(&stdout._Biobufhdr, "dbdumpmode\n");

	Bprint(&stdout._Biobufhdr, "type=%s, size=%s\n", mode->type, mode->size);
	Bprint(&stdout._Biobufhdr, "frequency=%d\n", mode->frequency);
	Bprint(&stdout._Biobufhdr, "x=%d (0x%X), y=%d (0x%X), z=%d (0x%X)\n",
		mode->x, mode->x, mode->y,  mode->y, mode->z, mode->z);
	Bprint(&stdout._Biobufhdr, "ht=%d (0x%X), shb=%d (0x%X), ehb=%d (0x%X)\n",
		mode->ht, mode->ht, mode->shb, mode->shb, mode->ehb, mode->ehb);
	Bprint(&stdout._Biobufhdr, "shs=%d (0x%X), ehs=%d (0x%X)\n",
		mode->shs, mode->shs, mode->ehs, mode->ehs);
	Bprint(&stdout._Biobufhdr, "vt=%d (0x%X), vrs=%d (0x%X), vre=%d (0x%X)\n",
		mode->vt, mode->vt, mode->vrs, mode->vrs, mode->vre, mode->vre);
	Bprint(&stdout._Biobufhdr, "hsync=%d, vsync=%d, interlace=%d\n",
		mode->hsync, mode->vsync, mode->interlace);

	for(attr = mode->attr; attr; attr = attr->next)
		Bprint(&stdout._Biobufhdr, "mode->attr: %s=%s\n", attr->attr, attr->val);
}
