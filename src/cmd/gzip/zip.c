...
#ifdef  QSH  //%-----------------------------------
STATIC int is_pathnamechar(char ch)
{
    if ((ch >= 'a' && ch <= 'z')  ||
        (ch >= 'A' && ch <= 'Z')  ||
        (ch >= '0' && ch <= '9')  ||
...
int qsh_zip(char *zfile,  char * files)
{
	int   fd, err;
	...
	Binit(&bout, fd, OWRITE);  //% getpid() is called !
	...
	//%	zip(&bout, argv[i], zfile == nil);
	{
		...
		for(j=0; (ch=*cp) && is_pathnamechar(ch); cp++)
		    srcfile[j++] = ch;
		srcfile[j] = 0;
		print(" zip<%s> ", srcfile);
		zip(&bout, srcfile, zfile == nil);
	    }
