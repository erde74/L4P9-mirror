...
char Rcmain[]="/t/rc/rcmain";   
//% char Rcmain[]="/rc/rcmain";     // /tmp/rc/rcmain
	...
	for(;path;path = path->next){
		...
		if(nc<1024){
			...
			if(nc+strlen(argv[1])<1024){
				...
				if (_DBGFLG){   //%
				    char *pp; int i=1; 
				    print("plan9.c::exec(%s ", file);  //%
				    while (pp = argv[i++])print(",%s ", pp);
				    print(")\n");
				} //%
				rc = exec(file, argv+1);
				DBGPRN("  XXEXEC():%d \n", rc); //%
