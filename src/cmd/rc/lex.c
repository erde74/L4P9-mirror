...
{ 
	...
	if(runq->iflag){
		pstr(err, promptstr);
		flush(err);   // Here, 'prompt' is printed
...
#if 1 //%-----------------------------
static int isprint(int c){if ((c >= ' ') && (c <= '~')) return 1; else return 0;}
#define LL(c)  //{if(runq->iflag && isprint(c))print("%c ",c);		\
               //	else if(c == '\n')print("NL ");else print("%s ", tok);} 
#endif //%----------------------------
