...
FF2
			...
			waitpid();
#else //%----------------------------------
			while(sleep(200), Waitfor(pid, 1) < 0)  //% sleep(299)
				;
#endif //%-----------------------------------
			DBGPRN("<Waitfor;Xsimple()\n");
		}
	}
FF2
}
...
{ 
	if(rp){
//DBGPRN(">doreadir(type=%d from=%d to=%d)\n", rp->type, rp->from, rp->to); //%
		doredir(rp->next);
		switch(rp->type){
		case ROPEN:   //% 1
			if(rp->from!=rp->to){
...
#if 1 //%   plan9: /dev/wdir  --> LP49: /tmp/dev/wdir ---
#define  WDIR  "/tmp/dev/wdir"   //%original "/dev/wdir"
static  char  PWD[128];
#else //% original---------------
#define  WDIR  "/dev/wdir"   
#endif //%------------------------------
