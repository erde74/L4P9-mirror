...
#define  _DBGFLG  0
#define  XX(ss)  {char xx[4];  print(ss); read(0, xx, 1);}  //%
...
main(int argc, char *argv[])
{
	...
	sleep(10); // Without sleep, it misbehaves ! Why ?

	kinit();
	//%	Trapinit(); // plan9.c: 514
	Vinit();
...
void
Xpipewait(void)
{  //L4_KDB_Enter("Xpipewait1");
	char status[NSTATUS+1];
	if(runq->pid==-1)
		setstatus(concstatus(runq->status, getstatus()));
	else{
		strncpy(status, getstatus(), NSTATUS);
		status[NSTATUS]='\0'; //L4_KDB_Enter("Xpipewait2");
		Waitfor(runq->pid, 1);
		runq->pid=-1;
...
{ 
	...
	if(flag['s'] && !truestatus())
		pfmt(err, "status=%v\n", vlook("status")->val);
	if(runq->iflag){   //l4printf_b("Xrdcmds  ");//%%
