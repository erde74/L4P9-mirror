...
#define DZZ(n) {static int cnt=0; if((++cnt)%(1024*n)==0)print("\n ZZZZ<%s>ZZZZ\n", __FUNCTION__);}
#define  IPCERR   error
#endif  //%----------------------------------------------

#define Channel void  // HK 20100131 
struct Ref {          // HK 20100131 
	long ref;
};


#if 1 //%--- moved from wind.c, xfid.c ---
enum { Wzero, 
       WKey, 
       WMouse, 
       WMouseread, 
       ...
       WWread, // NWALT, 
       Xfunc, 
       CWdata = WCwrite,   CWflush = 20, // NCW, 
       CRdata = WCread,    CRflush = CWflush, // NCR ,
       MRdata = WMouseread, MRflush = CWflush, // NMR, 
       WCRdata = WWread,   WCRflush = CWflush /*, NWCR*/ };
#endif //%-----------------------
...
typedef	struct	Ref Ref;	// HK 20100131
 ...
 **********************************************************/
struct Window  //% Window active object with winctl( ) 
{
        L_thcb        _a;      //% add
...
void		winctl(void*);
void		winshell(void*, void*);  //orig winshell(void*);
Window*	        wlookid(int);
Window*	        wmk(Image*, Mousectl*, int);
...
/**********************************************************
 *  Xfid active object / per fid                          *
 *       Data:     Xfid type                              *
 *       Action:   xfidctl() in xfid.c                    *
 **********************************************************/
struct Xfid  //%  Xfid active object with xfidctl() 
{
        L_thcb   _a;   //% add
 ...
 **********************************************************/
struct Filsys  //%  Filsys active objest with filsysproc()
{
        L_thcb  _a;  //% added
...
//% void	wctlproc(void*);
void	wctlthread_();  //% void	wctlthread(void*);

void	deletetimeoutproc(void*, char*);	// HK 20100312
...
/**********************************************************
 *  Mousectl active object / single object                *
 *       Data:     Mousectl type                          *
...
Mousectl	*mousectl;
Mouse	        *mouse;   //% mouse == &mousectl->_mouse
