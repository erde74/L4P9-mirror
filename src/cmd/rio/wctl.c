...
writewctl(Xfid *x, char *err)
{
	...
	cmd = parsewctl(&arg, rect, &rect, &pid, &id, &hideit, &scrollit, &dir, x->_fcall.data, err);  //%
	if(cmd < 0)
		return -1;
...
#if 1 //%----------------------
// "libactobj" uses kernel threads, thus wctlproc separation is unnecessary.
void
wctlthread_( )  
{
	char *buf;
...
wctlproc(void *v)
{
	...
	for(;;){
		...
		buf[n] = '\0';
		SENDP(wctl_aobj2, 0, buf); //%% 	sendp(c, buf);
	}
}
