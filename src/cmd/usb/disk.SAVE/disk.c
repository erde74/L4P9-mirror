...
umsinit(Ums *ums, int epin, int epout)
{
	...
	for(i = 0; i <= ums->maxlun; i++) {
		lun = &ums->lun[i];
		lun->_scsireq.lun = i;   //% _scsireq
		lun->_scsireq.umsc = lun;	//%	/* pointer to self */  
		  lun->_scsireq.flags = Fopen | Fusb | Frw10;  //%
		if(SRinquiry(&lun->_scsireq) == -1)    //%
			return -1;
		lun->inq = smprint("%.48s", (char *)lun->_scsireq.inquiry+8);  //%
		SRstart(&lun->_scsireq, 1);   //%
		if (SRrcapacity(&lun->_scsireq, data) == -1 &&   //%
		    SRrcapacity(&lun->_scsireq, data) == -1) {   //%
		        lun->blocks = 0; 
			lun->capacity = 0;
			lun->_scsireq.lbsize = 0; //%
		} else {
		        lun->_scsireq.lbsize = data[4]<<24|data[5]<<16|data[6]<<8|data[7]; //%
			lun->blocks = data[0]<<24|data[1]<<16|data[2]<<8|data[3];
			lun->blocks++; /* SRcapacity returns LBA of last block */
			lun->capacity = (vlong)lun->blocks * lun->_scsireq.lbsize;  //%
		}
	}
	return 0;
}
...
scanstatus(int ctlrno, int id)
{
	...
	if (debug)
		fprint(2, "\n%s: reading %s\n", argv0, buf);
	winner = 0;
	while (!winner && (p = Brdline(&f->_Biobufhdr, '\n')) != nil) {  //%
...
rread(Req *r)
{
	...
	case Qraw:
		...
		case Pcmd:
			...
			return;
		case Pdata:
		  lun->_scsireq.data.p = (uchar*)r->ofcall.data;  //%
			lun->_scsireq.data.count = r->ifcall.count;
			lun->_scsireq.data.write = 0;
			n = umsrequest(lun, &lun->_scsireq.cmd, &lun->_scsireq.data, &lun->_scsireq.status);  //%
			lun->phase = Pstatus;
			if (n == -1) {
				respond(r, "IO error");
				return;
			...
			break;
		case Pstatus:
		  n = snprint(buf, sizeof buf, "%11.0ud ", lun->_scsireq.status);  //%
			if (r->ifcall.count < n)
				n = r->ifcall.count;
	...
	case Qdata:
		...
		bno = r->ifcall.offset / lun->_scsireq.lbsize;  //%
		nb = (r->ifcall.offset + r->ifcall.count + lun->_scsireq.lbsize - 1) //%
		        / lun->_scsireq.lbsize - bno;  //%
		...
		if(nb * lun->_scsireq.lbsize > maxiosize)  //%
		  nb = maxiosize / lun->_scsireq.lbsize;  //%
		p = malloc(nb * lun->_scsireq.lbsize);   //%
		if (p == 0) {
			respond(r, "no mem");
			return;
		}
		lun->_scsireq.offset = r->ifcall.offset / lun->_scsireq.lbsize;  //%
		n = SRread(&lun->_scsireq, p, nb * lun->_scsireq.lbsize);  //%
		if(n == -1) {
			free(p);
			respond(r, "IO error");
			return;
		...
		len = r->ifcall.count;
		offset = r->ifcall.offset % lun->_scsireq.lbsize;  //%
		if(offset + len > n)
			len = n - offset;
		r->ofcall.count = len;
...
rwrite(Req *r)
{
	...
	case Qraw:
		...
		case Pcmd:
			...
			lun->_scsireq.cmd.p = lun->rawcmd;  //%
			lun->_scsireq.cmd.count = n;  //%
			lun->_scsireq.cmd.write = 1;  //%
			lun->phase = Pdata;
			break;
		case Pdata:
		        lun->_scsireq.data.p = (uchar*)r->ifcall.data;  //%
			lun->_scsireq.data.count = n;  //%
			lun->_scsireq.data.write = 1;  //%
			n = umsrequest(lun, &lun->_scsireq.cmd, &lun->_scsireq.data, &lun->_scsireq.status);  //%
			lun->phase = Pstatus;
			if(n == -1) {
				respond(r, "IO error");
	...
	case Qdata:
		...
		offset = r->ifcall.offset % lun->_scsireq.lbsize;  //%
		len = r->ifcall.count;
		if(offset || (len % lun->_scsireq.lbsize) != 0) {  //%
		  lun->_scsireq.offset = r->ifcall.offset / lun->_scsireq.lbsize;  //%
		  n = SRread(&lun->_scsireq, p, nb * lun->_scsireq.lbsize);  //%
			if(n == -1) {
				free(p);
		...
		lun->_scsireq.offset = r->ifcall.offset / lun->_scsireq.lbsize;  //%
		n = SRwrite(&lun->_scsireq, p, nb * lun->_scsireq.lbsize);  //%
		if(n == -1) {
			free(p);
			respond(r, "IO error");
			return;
