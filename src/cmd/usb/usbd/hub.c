...
static void _sysfatalimpl(char *fmt, va_list arg)
{
  char buf[1024];
...
portenable(Hub *h, int port, int on)
{
	if(h->isroot) {
	        if(fprint(h->portfd, "%s %d", on? "enable": "disable", port) < 0){
		        char path[32] = {'?', 0};            //%
			fd2path(h->portfd, path, 32);        //%
			print("-portenable{%H.%d h->portfd=%d path=%s port=%d}\n",
			      h, port, h->portfd, path, port); //%
			print("-fd[%d]<-{%s %d}\n", h->portfd, on? "enable": "disable", port); //%
...
portreset(Hub *h, int port)
{
	if(h->isroot) {
	        if(fprint(h->portfd, "reset %d", port) < 0){ 
		        char path[32] = {'?', 0};            //%
			fd2path(h->portfd, path, 32);        //%
			print("-portreset{%H.%d h->portfd=%d path=%s port=%d}\n", 
			      h, port, h->portfd, path, port); //%
			print("-fd[%d]<-{reset %d}\n", h->portfd, port);  //%
...
portstatus(Hub *h, int port)
{
	...
	char  path[32] = {'?', 0};
	DBGPRN(">portstatus(port=%d)\n", port);

	if(h->isroot) {
		seek(h->portfd, 0, 0);
		status = malloc(1024);   //% original-is 8192

		n = read(h->portfd, status, 1024);   //% original: 8192
       		if (n <= 0) {
		        fd2path(h->portfd, path, 32);  //%
			PRN("-portstatus{fd=%d path=%s}\n", h->portfd, path); //%
