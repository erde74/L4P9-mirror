...
{
    uint  *stk = (uint*)malloc(stacksize);
    uint  *sp;
    L4_ThreadId_t  tid;
...
#define STACKSIZE  (20*1024)    //%  128*1024
...
void
main(int argc, char **argv)  //% threadmain
{
	int i;
	Hub *h;
...
configure(Hub *h, int port)
{
	...
	DBGPRN("configure-maxpkt=%d\n", maxpkt);
	if(maxpkt < 0) {
Error0:
	        portenable(h, port, 0);
		DBGPRN("<configure():nil\n");
		return nil;
	}

	d = opendev(h->ctlrno, -1);
	DBGPRN("configure-opendev=%x\n", d);
