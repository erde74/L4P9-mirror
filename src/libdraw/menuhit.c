#include <l4all.h>      // HK 20091231
#include <u.h>
#include <libc.h>
#include <draw.h>
//#include <thread.h>
#include <l_actobj.h>	// HK 20091231
#include <mouse.h>
...
menuscan(Image *m, Menu *menu, int but, Mousectl *mc, Rectangle textr, int off, int lasti, Image *save)
{
	...
	for(readmouse(mc); mc->_mouse.buttons & (1<<(but-1)); readmouse(mc)){ //%
	  i = menusel(textr, mc->_mouse.xy);  //%
		if(i != -1 && i == lasti)
			continue;
...
menuhit(int but, Mousectl *mc, Menu *menu, Screen *scr)
{
	...
	while(mc->_mouse.buttons & (1<<(but-1))){  //%
		lasti = menuscan(b, menu, but, mc, textr, off, lasti, save);
		if(lasti >= 0)
			break;
		while(!ptinrect(mc->_mouse.xy, textr) && (mc->_mouse.buttons & (1<<(but-1)))){ //%
		  if(scrolling && ptinrect(mc->_mouse.xy, scrollr)){ //%
		                noff = ((mc->_mouse.xy.y-scrollr.min.y)*nitem)/Dy(scrollr); //%
				noff -= nitemdrawn/2;
				if(noff < 0)
					noff = 0;
