...
announce(char *addr, char *dir)
{
  int ctl, n, m, rcode;
	...
	DBGPRN(">nettrance() addr=%s naddr=%s netdir=%s\n", addr, naddr, netdir);

	/*
	 * get a control channel
	 */
	ctl = open(netdir, ORDWR);   
	DBGPRN("open(\"%s\", ORDWD):%d\n", netdir, ctl);
	if(ctl<0){
		werrstr("announce opening %s: -- \n", netdir);
		return -1;
	}
	...
	*cp = 0;
	...
	rcode = write(ctl, buf2, n);
	DBGPRN("write(%d, \"%s\"):%d\n", ctl, buf2, rcode);
	if(rcode!=n){
		werrstr("announce writing %s: --\n", netdir);
		close(ctl);
		return -1;
	}
...
listen(char *dir, char *newdir)
{
	...
	ctl = open(buf, ORDWR);
	DBGPRN("listen:open(\"%s\", ORDWD):%d\n", buf, ctl);

	if(ctl < 0){
		werrstr("listen opening %s: --\n", buf);
		return -1;
	}
	...
	*++cp = 0;
	...
	if(m <= 0){
		close(ctl);
		werrstr("listen reading %s/listen: --\n", dir);
		return -1;
	}
	buf[n+m] = 0;
